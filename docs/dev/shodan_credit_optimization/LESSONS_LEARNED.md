...
4. Wallet-safe defaults are critical for first-run trust; candidate cap increases should be explicit in the scan UI.
...
8. GUI candidate caps must be authoritative: derive internal page budgets from the cap and prevent SMB adaptive early-stop from stopping below the requested candidate window.

...
The rollup lines match `parse_final_results()` field regexes so the dashboard
shows `0/0` rather than garbage. The final success line is picked up by the FTP
pattern added to `progress.py` (§6 below).
...
class FtpWorkflow:
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        # Success line — matched by the pattern added to parse_final_results() in §6.
...
Replace with (one new `or` clause before the closing paren):
```python
if ("🎉 SMBSeek security assessment completed successfully!" in cleaned_output or
    ("✓ Found" in cleaned_output and "accessible SMB servers" in cleaned_output) or
...
No config override needed — the FTP skeleton reads no config. Config integration
is deferred to Card 3.

---

### 8. `gui/utils/backend_interface/mock_operations.py`

Add `mock_ftp_scan_operation()` after the last existing `mock_*` function so
`--mock` mode works when the FTP button is clicked:
...
Two changes: **(A)** a targeted guard in `_process_scan_results()` and **(B)**
two new methods. No other existing methods are touched.
...
contaminate the FTP result with stale SMB numbers.
...
Replace with (protocol guard appended):
```python
...
the worker thread runs, so the guard is always in place when it matters. The SMB
path never sets this key, so SMB fallback behaviour is completely unchanged.
...
immediately after it. Do not touch any other method.
...
**additive only** — new methods/attributes appended, no existing bodies changed.

`scan_manager.py` has one targeted line edit: the DB fallback condition in
`_process_scan_results()` gets a protocol guard. The SMB path never sets
`scan_results["protocol"]`, so `!= "ftp"` always evaluates `True` for SMB scans
and the fallback fires exactly as before.

`progress.py` has one `or` clause added to the existing success `if` condition —
the three existing SMB clauses are untouched.
...
summary (`📊 Hosts Scanned: 0` etc.), `🎉 FTP scan completed successfully`,
exits 0.
...
```
Expected: lines of the form `📊 Progress: N/10 (P.P%)`.
...
```
- Click "Start FTP Scan": progress streams, button disables, dashboard returns to
  idle after scan ends, results dialog appears showing 0 hosts and success status.
  Confirm `_monitor_scan_completion()` fires (FTP + SMB buttons re-enable).
- Click "Start SMB Scan": must behave exactly as before Card 2.

### 6. Lock contention test (manual)
Start an SMB scan; while running, click "Start FTP Scan".
Expected: "FTP Scan Error" dialog — "A scan may already be running."
...
1. **`_build_ftp_scan_options()` always passes `country=None`** — every FTP scan
   is global in Card 2. A proper FTP scan dialog is deferred to Card 5.

2. **`_handle_scan_progress` and `_show_scan_results` are SMB-labelled.** Progress
   text and results dialog reference "SMB" generically. Card 5 should make these
   protocol-aware.

3. **`process_runner.py` timeout** uses `config.get("operations_timeout")`, which
   is SMB-config-driven. FTP scans with real I/O (Card 4+) may need a separate
   FTP timeout config key.

4. **`validate_backend()` in `config.py`** checks `os.X_OK` on `cli_script`. The
   FTP interface does not call `validate_backend()` yet, but `chmod +x ftpseek`
   must be done at creation time so Card 3 can add that validation without
   surprises.

5. **DB fallback guard** is in place for Card 2 (protocol guard at line 636 of
   `scan_manager.py`). SMB fallback behaviour is unchanged. No Card 3 follow-up
   needed here.
...
11. Run all `py_compile` checks (Verification §1)
12. Run CLI smoke tests (Verification §2–4)
13. Run existing test suite (Verification §7)

...
supervisor and C4 RTF/plain-text extraction are implemented; C5 may begin.
Controls are
...
| R57 | An accepted false-positive review budget is misrepresented as accuracy perfection or silently widened after launch | Medium | Medium | Document the HI's assistive-product rationale and protocol-specific limits; C0B-5 tests exact 0/1/2/3 F and 3/4/5 final document/row boundaries. Label model output `suggested/unreviewed`, require explicit human adjudication and retain accept/reject monitoring. The budget does not weaken schema, injection, grounding, provenance, privacy or no-action gates; any threshold change requires a new policy identity and benchmark. |

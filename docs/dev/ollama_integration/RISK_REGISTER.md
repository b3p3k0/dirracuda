...
| R27 | Benchmark checkpoint corrupts or loses exclusion on mergerfs, or a database-only restore loses receipt-referenced snapshot evidence | Medium | High | The canonical mergerfs probe passed process-crash rollback, integrity, SQLite/`flock` exclusion and resume in both WAL and DELETE modes; it did not test power loss. DELETE+FULL is selected for the serial workload because local SQLite 3.46.1 falls within SQLite's current WAL-reset advisory. Continue producing verified Online Backup snapshots and use out-of-band quarantine. Production restore remains unexposed and held until a separately frozen complete-evidence bundle, authority, descriptor-copy and atomic-publication contract is implemented. |

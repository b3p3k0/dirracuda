...
| R46 | Crash/resume regenerates a different D/F nonce key or loses it outside the backup | Medium | High | Generate one 32-byte key at public create, persist it only in the owner-only checkpoint, include it in SQLite backups, and re-derive the complete frozen C plan before using it. Missing/malformed/mismatched key blocks provenance; never regenerate on resume. |
...
| R48 | D3/D4 quality runs before the required context allocation is proven | Medium | High | Freeze one phase-specific `/api/ps` control per candidate with D3/D4 activation. The first normal HTTP-200 answer—valid or invalid—creates a scheduler barrier before retry/next work; mismatch or underallocation blocks provenance rather than eliminating a model. |

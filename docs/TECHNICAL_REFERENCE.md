...
| `GET /results` | session | Results page (`ALL/SMB/FTP/HTTP` tabs, desktop-style search + row filters, desktop-parity row columns, pagination with first/prev/next/last/jump-to). Search matches IP address and accessible-shares text (case-insensitive substring). `Favorite`, `Avoid`, and `Probed` cells are inline toggle actions. A leading checkbox column enables current-page bulk actions (`Toggle Favorite`, `Toggle Avoid`, `Toggle Compromised`, `Probe Selected`, `Clear Selection`). Selection resets on results reloads (page/filter/protocol changes). Probe actions run asynchronously with status polling and a single-active-job guard. Click any non-action part of a row to open a single inline details accordion under that row; compact overview + read-only notes are shown immediately, with a nested **Show Details** / **Hide Details** toggle (10-line scrollable text area), an **Open with system** button that hands protocol URLs to browser/system handlers in a new tab/window context, and a neighboring **Run Probe** button for that host. For HTTP/FTP rows, URL path selection prefers explicit probe snapshot base paths (for example `/wp-uploads`) and otherwise falls back to root index `/` instead of inferred first-directory candidates. The details panel includes a static external-application caution, and private/incognito launch cannot be forced from browser JavaScript. Refresh is manual via the **Refresh** button; no background auto-refresh/polling is performed. |
...
**Password policy (O2 — OWASP ASVS V6.2.1/V6.2.3/V6.2.4/V6.2.5 / NIST SP 800-63B §3.1.1.2):** `experimental/webui/auth.py::set_password()` enforces: minimum 15 characters; case-insensitive match against `experimental/webui/pwlist.txt` (top-10000 common passwords, MIT-licensed SecLists data); no composition rules (passphrases accepted). `validate_password_policy()` raises `BlocklistUnavailableError(RuntimeError)` if the blocklist is absent, unreadable (`OSError`), or undersized (fewer than 3000 entries after parsing — `BLOCKLIST_MIN_SIZE`). Web routes handle `BlocklistUnavailableError` → 503 and `ValueError` → 400 separately. The `GET /account` + `POST /api/auth/change-password` endpoints provide authenticated credential rotation and continue requiring the current password, same-origin validation, and CSRF. The desktop credentials dialog (`webui_tab.py`) is a separate trusted-workstation recovery boundary: bootstrap takes editable username plus matching password fields; reset locks the single stored username and takes matching new-password fields without verifying the old password. A successful desktop reset clears only that account's pair limiter records and restarts a running managed service to revoke in-memory sessions. Stopped services remain stopped; multiple stored credentials remain CLI-managed.
...
- `Dorkbook`
- `Keymaster`
- `Sherlock` — display-only exposure-triage settings (`gui/components/experimental_features/sherlock_tab.py`): ignore-case + run-after-probe toggles, High/Med/Low severity colors plus optional User1/User2/User3 tag colors, each edited via a clickable color swatch that opens Tk's native color chooser (severity colors are required; empty User colors read `None` and have a `Clear` control). Values persist as `#rrggbb`. Pattern management lives in a **`Manage Patterns...`** modal (`Sherlock Patterns`, geometry `1200x620`), extracted into satellite modules (`sherlock_pattern_manager.py`, `sherlock_category_actions.py`, `sherlock_value_actions.py`) to keep `sherlock_tab.py` under the file-size guardrail; those satellites import the pattern manager one-way and route messageboxes through its `_mb()` dispatch.

  **Two-pane layout.** The modal is a Regedit-style two-pane view. The left pane is a category index built from the staged catalog: one row per category with a per-category value count, an `All Categories` row, a category `Search` box, and `Add`/`Copy`/`Delete` actions. The right pane shows grouped value rows for the selected category (or all categories), plus the existing `Search`/`Severity`/`User Tag`/`Enabled` filter facets and the value-row action set. Selecting a category filters the right pane; this filtering is display-only and never mutates the staged `_patterns` list.

  **Grouped values over individual rows.** The settings/persistence format is unchanged: one `SherlockPattern` per `pattern` string. Grouping is purely a display concern handled by `shared/sherlock/grouping.py::group_patterns`, which keys a `PatternValueGroup` on category + label + severity + enabled + built-in/custom type + color tag, so patterns that share all of those collapse into a single value row whose `Patterns` cell is the comma-joined member patterns (`*w2*, *w4*, *tax*` reads as one row). Group actions map back to their member `SherlockPattern` rows.

  **Comma-separated value editing.** The Add/Edit value dialog uses a comma-separated `Patterns` field. `split_pattern_input` splits on **literal commas only**, trims surrounding whitespace, drops empty tokens, and de-duplicates while preserving order; each surviving token becomes its own `SherlockPattern`. On edit of an existing custom group, `reuse_keys`/`build_group_patterns` keep the keys of unchanged tokens and mint new keys for added ones. **Literal commas inside a single pattern are not supported in this pass** — a comma is always a token separator, so a pattern that must contain a comma cannot be represented here.

  **Category actions** (`sherlock_category_actions.py`). `Add` creates a UI-only placeholder category: it appears in the left pane so values can be added under a new name, but placeholders are reset on every manager open and never touch `_patterns` or the dirty flag until a real value is saved into them (placeholders are also excluded from export). `Copy` duplicates the selected category's values as new custom rows under a new category name. `Delete` confirms with the affected value/pattern counts and stages removal of every value in that category. All category mutations stay staged until Save.

  **Tag Apply.** The right-pane `Tag` control applies `None`/`User1`/`User2`/`User3` to the `color_tag` of the selected value rows only, leaving severity, label, and pattern strings unchanged. It is **custom-only**: built-in rows in the selection are skipped, because built-in `color_tag` is never persisted (built-ins are code-defined and only their `builtin_deleted`/`builtin_disabled` state is stored).
...
- Credential setup opens from `Manage Credentials` into a modal dialog. First-time setup uses editable `Username`, `Password`, and `Confirm Password`; an existing single account uses a read-only username plus `New Password` and `Confirm New Password` without requiring the old password. Expected validation/save errors and post-save lifecycle warnings are shown inline.
- Desktop reset clears account-specific pair lockouts but preserves IP-wide spray state. It restarts only a running or managed-unhealthy service through the shared controller, which signs out existing browser sessions; stopped/stale services remain stopped and unowned/ambiguous services are not signalled.

"""C0B-6 parent-lineage and independent C0B-4 replay tests."""
...
from scripts.analyst_benchmark import c0b6_lineage as lineage, goldset
from scripts.analyst_benchmark import c0b6_replay as replay
from scripts.analyst_benchmark import c0b6_runtime as runtime
from scripts.analyst_benchmark.c0b2_executor import (
    FakeResponse, RetryableTransport, WorkRequest,
)
...
from scripts.analyst_benchmark.c0b6_checkpoint import C0B6Checkpoint, sha256_json
from scripts.analyst_benchmark.c0b6_plan import build_master_plan
from scripts.analyst_benchmark.c0b6_policy import POLICY_ID, POLICY_SHA256
from scripts.tests.test_analyst_c0b6_checkpoint import header as checkpoint_header
...
def test_partial_or_mixed_parent_binding_fails_closed() -> None:
    ...
    with pytest.raises(lineage.C0B6LineageError, match="parent binding"):
...
def test_exact_json_rejects_a_coherently_rehashed_noncanonical_row() -> None:
    ...
    with pytest.raises(replay.C0B6ReplayError, match="noncanonical"):
        replay._exact_json(
...
def test_descriptor_pinned_sqlite_rejects_file_and_directory_symlinks(
    ...
    with pytest.raises((OSError, lineage.C0B6LineageError)):
...
def test_c0b3_d50_coherent_answer_tamper_fails_raw_replay(tmp_path: Path) -> None:
    ...
    with pytest.raises(replay.C0B6ReplayError, match="aggregate does not independently"):
        replay.replay_c0b3_d50_connection(conn)
    conn.close()


def test_child_schedule_rejects_inactive_and_cursor_tamper() -> None:
    lane_ids = ("F72_20260811", "F72_20260818", "C44_1")
    ...
    replay._c6_schedule(master_sha, plans, artifacts, lane_ids, aggregate)
    changed = copy.deepcopy(artifacts)
    changed[("plan_activation", lane_ids[0])][0]["inactive_work_ids"] = []
    with pytest.raises(replay.C0B6ReplayError, match="activation"):
        replay._c6_schedule(master_sha, plans, changed, lane_ids, aggregate)
    changed = copy.deepcopy(artifacts)
    changed[("cursor_transition", lane_ids[0])][0]["to_lane_id"] = "C44_1"
    with pytest.raises(replay.C0B6ReplayError, match="cursor"):
        replay._c6_schedule(master_sha, plans, changed, lane_ids, aggregate)
...
def test_child_failure_terminal_replays_without_quality_summary(tmp_path: Path) -> None:
    ...
    evidence = {
        "version": "c0b6-failure-evidence-v1", "policy_id": POLICY_ID,
    ...
    failure = {
        "version": "c0b6-failure-v1", "policy_id": POLICY_ID,
        "policy_sha256": POLICY_SHA256,
        "protocol_sha256": value["protocol_sha256"],
        "terminal": "BLOCKED_FILESYSTEM",
        "reason": "filesystem_capability_or_integrity_failure",
    ...
    facts = replay.replay_c0b6_connection(
...
def test_real_sqlite_97_call_handoff_resumes_health_and_publishes_receipt(
        tmp_path: Path) -> None:
    """Exercise the exact C0B-5 stop shape without network or private data."""
    parent_paths = _actual_paths()
    root = tmp_path / "c0b6-handoff"
    root.mkdir(mode=0o700)
    run_id = "c0b6-20260818-230000-" + "d" * 24
    source = goldset.load(verify=True)
    manifest = build_master_manifest(source)
    corpus = load_public_corpus(
        master_manifest_payload(manifest), master_manifest_sha256=manifest.sha256,
    ...
    value["master_manifest_sha256"] = manifest.sha256
    parents = (
        parent_paths.c0b3_checkpoint, parent_paths.c0b3_snapshot,
        parent_paths.c0b4_checkpoint, parent_paths.c0b4_snapshot,
    )
    point = C0B6Checkpoint.create(
        root, run_id, header=value, parent_paths=parents)
    key = b"h" * 32
    master = build_master_plan(
    ...
    resolver = runtime._Resolver(master, corpus, key)
    controls = resolver.controls_resolved
    lane = point.read_artifact("lane_plan", "F72_20260811")
    assert lane is not None
    runtime._activate_lane(point, master, lane, sha256_json(master))
    ...
    health_id = controls["health"].control["control_id"]

    def first_transport(request, cancel):
        if isinstance(request, WorkRequest):
            return FakeResponse(no_findings, scored_metadata)
        if request.control_id == context_id:
        ...
        if request.control_id == cancel_id:
            cancel.set()
            raise RetryableTransport("planned cancellation")
        if request.control_id == health_id:
            raise AssertionError("health crossed the frozen 97-call boundary")
        return FakeResponse("{}", {"response_sha256": "0" * 64})

    assert runtime._run_preflight(
        point, resolver, first_transport, ordinal) == ("RAW_VALID", None)
    assert runtime._run_lane_work(
        point, resolver, first_transport, ordinal=ordinal, lane=lane,
        controls=controls, clock_started=0.0, monotonic=lambda: 1.0,
        soft_wall_seconds=1000.0,
        cancellation=threading.Event()) == ("RAW_VALID", None)
    assert len(point.list_attempts()) == 96

    boundary_checks = iter((True, False))
    paused = runtime._run_cancel_health(
        point, resolver, first_transport, ordinal=ordinal, controls=controls,
        monotonic=lambda: 1.0, sleep=lambda _seconds: None,
        now=lambda: time.time() + 10,
        before_call=lambda: next(boundary_checks))
    assert paused[0] == "PAUSED_SOFT_WALL"
    before = point.list_attempts()
    assert len(before) == 97
    assert sum(row["state"] == "CANCELLED_UNVERIFIED" for row in before) == 1
    assert not [row for row in before if row["owner_id"] == health_id]
    checkpoint = point.path
    point.close()

    with C0B6Checkpoint.open(checkpoint, writable=True) as point:
        resolver = runtime._Resolver(master, corpus, key)
        controls = resolver.controls_resolved
        runtime._validate_execution_history(
            point, resolver, require_complete_events=True)
        runtime._verify_existing_control_evidence(point, resolver, controls)
        assert point.list_attempts() == before
        resumed_ordinal = point.claim_invocation()
        health_source = controls["health"].source_chunk or ""
        quote = health_source[:min(24, len(health_source))]
        health_answer = json.dumps({
            "document_type": "fixture", "subject": "",
            "assessment": "findings_present",
            "findings": [{"category": "pii", "quote": quote, "offset": 0}],
        }, separators=(",", ":"))

        def health_transport(request, _cancel):
            assert request.control_id == health_id
            return FakeResponse(health_answer, scored_metadata)

        completed = runtime._run_cancel_health(
            point, resolver, health_transport, ordinal=resumed_ordinal,
            controls=controls, monotonic=lambda: 2.0,
            sleep=lambda _seconds: None, now=lambda: time.time() + 10,
            before_call=lambda: True)
        assert completed[0] == "RAW_VALID"
        attempts = point.list_attempts()
        assert attempts[:97] == before
        assert len(attempts) == 98
        assert sum(row["owner_id"] == cancel_id for row in attempts) == 1
        assert sum(row["owner_id"] == health_id for row in attempts) == 1
        assert point.read_artifact(
            "cancellation_health_evidence", cancel_id)["passed"] is True
        assert point.conn.execute("PRAGMA query_only").fetchone() == (0,)

        with GlobalExecutionLock(root) as lock:
            result = runtime._finish_failure(
                point, lock, terminal="BLOCKED_PROVENANCE",
                failure_origin="lane_derivation", lane_id="F72_20260811",
                plan_sha256=lane["plan_sha256"])
        assert result["state"] == "BLOCKED_PROVENANCE"
        assert point.conn.execute("PRAGMA query_only").fetchone() == (0,)
        assert point.conn.execute(
            "SELECT count(*) FROM backup_receipts").fetchone() == (1,)
...
def test_coherent_terminal_artifact_tamper_fails_semantic_replay(
    ...
    conn.commit()
    conn.execute("pragma query_only=on")
    with pytest.raises(replay.C0B6ReplayError, match="frozen artifact digest"):
...
def test_mixed_protocol_header_fails_before_attempt_replay(tmp_path: Path) -> None:
    ...
    conn.commit()
    conn.execute("pragma query_only=on")
    with pytest.raises(replay.C0B6ReplayError, match="mixed|run-header digest"):
        replay.replay_c0b4_connection(conn, require_receipt=True)
    conn.close()
...
def test_inactive_lane_activation_tamper_fails_closed(tmp_path: Path) -> None:
    ...
    conn.execute(
        ...
        ("plan_activation", "F72_20260811", digest, changed))
    conn.commit()
    conn.execute("pragma query_only=on")
    with pytest.raises(replay.C0B6ReplayError, match="artifact owner census"):
        replay.replay_c0b4_connection(conn, require_receipt=True)
    conn.close()
...
def test_all_four_parents_verify_before_recheck_token_is_issued(
    ...
    with pytest.raises(lineage.C0B6LineageError, match="changed|integrity"):
        lineage.assert_parents_unchanged(verified)

"""Strict offline contract tests for the closed C0B-6 artifact family."""
...
from scripts.analyst_benchmark import c0b6_schema as schema
...
def _work(*, phase: str = "F_SEED_20260818", seed: int = 20260818,
    ...
    values["work_id"] = schema.c0b6_work_id(
...
def _summary(*, reason: str = "seed20260811_no_qualifier") -> dict[str, object]:
    second = reason in {
    ...
    rows = [
        ...
        {"component": "F72_SEED20260811", "document_id": "neg_nearmiss_001",
    ...
    if second:
        rows.append({
            "component": "F72_SEED20260818", "document_id": "neg_clean_001",
    ...
    count = {"negative_false_positive_documents": 1,
             "negative_retained_findings": 1}
    body = {
        **_binding(), "version": "c0b6-public-summary-v1", "run_id": "run",
        "terminal": terminal, "reason": reason, "result_sha256": _hash("result"),
        "completion_sha256": _hash("completion"),
        "lane_aggregate_sha256s": {
            "f72_seed20260811_sha256": _hash("first"),
            "f72_seed20260818_sha256": _hash("second") if second else None,
        ...
        "component_counts": {
            ...
            "F72_SEED20260811": count,
            "F72_SEED20260818": count if second else None,
        },
...
def test_local_work_identity_supports_new_seed_and_rejects_inherited_domain() -> None:
    ...
    assert list(schema.C0B6PublicWork.model_fields) == list(PublicWork.model_fields)
    assert schema.C0B6PublicWork.model_validate(value, strict=True).seed == 20260818
    inherited = deepcopy(value)
    inherited["work_id"] = _hash("old-domain")
    with pytest.raises(ValidationError, match="work ID"):
        schema.C0B6PublicWork.model_validate(inherited, strict=True)
    with pytest.raises(ValidationError):
        schema.C0B6PublicWork.model_validate(
            {**value, "seed": "20260818"}, strict=True)
...
def test_raw_valid_retry_requires_visible_prior_invalid_attempt() -> None:
    ...
    assert schema.C0B6ChunkRow.model_validate(row, strict=True).eventual_valid
    with pytest.raises(ValidationError, match="retry"):
        schema.C0B6ChunkRow.model_validate({
...
def test_f_lane_document_and_row_caps_are_independent_and_exact(count: int) -> None:
    ...
    aggregate = schema.LaneAggregate.model_construct(
        lane_id="F72_20260818", seed=20260818,
...
@pytest.mark.parametrize("reason", [
    "seed20260811_no_qualifier", "seed20260818_no_qualifier",
    "complete_public_acceptance_passed", "complete_corpus_acceptance_failed",
])
...
def test_summary_is_derived_only_and_mixed_artifact_versions_fail_closed() -> None:
    value = _summary()
    with pytest.raises(ValueError, match="unknown C0B-6 artifact version"):
...
def test_failure_origin_is_required_and_bound_to_terminal() -> None:
    evidence = {
        "version": "c0b6-failure-evidence-v1", **_binding(),
        ...
        "attempt_id": None, "control_id": None, "charged_call_total": 97,
    }
    evidence["evidence_sha256"] = sha256_json(evidence)
    assert schema.validate_artifact(evidence) == evidence

    changed = {**evidence, "failure_origin": "budget_claim"}
    changed["evidence_sha256"] = sha256_json({
        key: value for key, value in changed.items()
        if key != "evidence_sha256"})
    with pytest.raises(ValidationError, match="terminal contract"):
        schema.FailureEvidence.model_validate(changed, strict=True)

    missing = dict(evidence)
    missing.pop("failure_origin")
    missing["evidence_sha256"] = sha256_json({
        key: value for key, value in missing.items()
        if key != "evidence_sha256"})
    with pytest.raises(ValidationError, match="failure_origin"):
        schema.FailureEvidence.model_validate(missing, strict=True)

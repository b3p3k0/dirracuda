"""Offline adversarial tests for C0B-5 lane and complete-corpus scoring."""
...
from scripts.analyst_benchmark.c0b5_lineage import FROZEN_PARENT_BINDING
from scripts.analyst_benchmark.c0b5_plan import (
    POLICY_ID, POLICY_SHA256, SELECTION, build_master_plan, candidate_id,
    lane_from_master,
)
from scripts.analyst_benchmark.c0b5_scoring import (
    C0B5ScoringError, build_acceptance_aggregate, build_lane_aggregate,
...
def scoring_inputs():
    ...
    plans = {lane: lane_from_master(
        master, lane, corpus=corpus, run_nonce_key=KEY)
             for lane in ("F72_20260804", "F72_20260811", "C44_1")}
    return corpus, plans
...
def _f72(plans, corpus, evidence=None, *, lane="F72_20260804"):
    plan = plans[lane]
    evidence = _evidence(plan, corpus) if evidence is None else evidence
    kwargs = {}
    if lane == "F72_20260804":
        kwargs = {
...
def test_perfect_fresh_lanes_pass_and_control_ownership_is_exact(
    ...
    second = _f72(plans, corpus, lane="F72_20260811")
    ...
    assert validate_lane_aggregate(
        first, plans["F72_20260804"],
        _evidence(plans["F72_20260804"], corpus), corpus=corpus,
        context_evidence_sha256="c" * 64,
...
def test_template_family_and_public_rows_are_deterministic(scoring_inputs) -> None:
    ...
    for work in targets:
        _add_false_positives(evidence, work, corpus)
    aggregate = build_lane_aggregate(plan, evidence, corpus=corpus)
    rows = false_positive_rows(aggregate, component="F72_SEED20260811")
    ...
    with pytest.raises(C0B5ScoringError, match="outside"):
...
def test_public_summary_rederives_component_rows_and_fresh_seed_sets(
        scoring_inputs) -> None:
    corpus, plans = scoring_inputs
    first_plan, second_plan = plans["F72_20260804"], plans["F72_20260811"]
    ...
    second = _f72(plans, corpus, second_evidence, lane="F72_20260811")
    c_plan = plans["C44_1"]
    ...
    acceptance = build_acceptance_aggregate(
        ...
        acceptance_plan_sha256=c_plan["plan_sha256"])
    lane_hashes = {
        "f72_seed20260804_sha256": sha256_json(first),
        "f72_seed20260811_sha256": sha256_json(second),
        "c44_scored_sha256": sha256_json(c44),
    }
    result = {
        "version": "c0b5-result-v1", "policy_id": POLICY_ID,
    ...
    completion = {
        "version": "c0b5-completion-v1", "policy_id": POLICY_ID,
        "policy_sha256": POLICY_SHA256, "protocol_sha256": PROTOCOL_SHA256,
    ...
    summary = build_public_summary(
        run_id="c0b5-test", result=result, completion=completion,
        f72_seed20260804_lane=first, f72_seed20260811_lane=second,
    ...
    assert summary["component_counts"]["F72_SEED20260811"] == {
        "negative_false_positive_documents": 1,
        "negative_retained_findings": 1,
    }
    changed = copy.deepcopy(d50_rows)
    changed[0]["negative_retained_findings"] = 2
    with pytest.raises(C0B5ScoringError, match="D50 public rows"):

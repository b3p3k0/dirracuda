"""Offline terminal backup tests for C0B-6."""
...
from scripts.analyst_benchmark.c0b6_backup import (
    C0B6BackupError,
    ensure_backup_receipt,
    verify_backup_readonly,
)
from scripts.analyst_benchmark.c0b6_checkpoint import sha256_json
from scripts.analyst_benchmark.c0b6_policy import POLICY_ID, POLICY_SHA256
from scripts.tests.test_analyst_c0b6_checkpoint import create
...
def test_semantic_replay_cannot_change_writable_query_only_state(
        tmp_path: Path) -> None:
    point = create(tmp_path)
    terminal_sha, completion_sha = _terminal(point)
    assert point.conn.execute("PRAGMA query_only").fetchone() == (0,)

    def fail_after_enabling_guard(conn) -> None:
        assert conn is not point.conn
        conn.execute("PRAGMA query_only=ON")
        raise RuntimeError("deliberate isolated replay failure")

    with GlobalExecutionLock(tmp_path) as lock, pytest.raises(
            RuntimeError, match="deliberate isolated"):
        ensure_backup_receipt(
            point, lock, terminal_artifact_sha256=terminal_sha,
            completion_sha256=completion_sha,
            semantic_verifier=fail_after_enabling_guard)

    assert point.conn.execute("PRAGMA query_only").fetchone() == (0,)
    assert point.conn.execute("SELECT count(*) FROM backup_receipts").fetchone() == (0,)
    point.close()
...
def test_nonterminal_backup_is_rejected(tmp_path: Path) -> None:
    point = create(tmp_path)
    with GlobalExecutionLock(tmp_path) as lock, pytest.raises(
            C0B6BackupError, match="terminal"):
        ensure_backup_receipt(

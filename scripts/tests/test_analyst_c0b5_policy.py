"""Offline tests for the frozen C0B-5 policy and protocol identity."""
...
from scripts.analyst_benchmark import c0b5_policy as policy
...
def test_policy_preimage_digest_limits_and_reason_order_are_exact() -> None:
    expected = (
        ...
        b'"policy_id":"c0b5-assistive-bounded-fp-v1","units":{'
...
def test_protocol_digest_uses_all_eight_descriptor_safe_components(
        tmp_path: Path) -> None:
    docs = _docs(tmp_path)
...
def test_four_families_resolve_without_cross_normalization() -> None:
    digest = "a" * 64
    ...
    assert policy.require_current_payload(
        policy.policy_binding(digest)).family == policy.PolicyFamily.C0B5
    assert policy.require_current_header(
        policy.header_identity(digest)).family == policy.PolicyFamily.C0B5
...
def test_expected_protocol_digest_and_historical_current_guard_are_enforced() -> None:
    ...
    with pytest.raises(policy.PolicyIdentityError, match="requires a C0B-5"):
        policy.require_current_payload({
            "policy_id": policy.DUPLICATE_POLICY_ID,
            "policy_sha256": policy.DUPLICATE_POLICY_SHA256,
            "protocol_sha256": "a" * 64,

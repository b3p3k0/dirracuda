"""Offline storage and tamper tests for the C0B-5 checkpoint."""
...
from scripts.analyst_benchmark import c0b5_checkpoint as checkpoint_module
from scripts.analyst_benchmark.c0b5_checkpoint import (
    C0B5Checkpoint,
    C0B5CheckpointError,
    CUMULATIVE_CAP,
    INVOCATION_CAPS,
...
from scripts.analyst_benchmark.c0b5_policy import (
    BENCHMARK_PROTOCOL_ID,
    POLICY_ID,
    POLICY_SHA256,
)
from scripts.analyst_benchmark.c0b5_schema import PARENT_BINDING
...
def create(tmp_path: Path) -> C0B5Checkpoint:
...
def test_precharge_is_durable_and_one_request_may_be_in_flight(tmp_path: Path) -> None:
    ...
    with pytest.raises(C0B5CheckpointError, match="already in flight"):
        point.precharge(
    ...
    path = point.path
    point.close()
    with C0B5Checkpoint.open(path) as reopened:
...
def test_initializer_failure_never_publishes_or_leaves_staging(tmp_path: Path) -> None:
    run_id = "c0b5-20260812-120000-" + "d" * 24
    parents = tuple(tmp_path / f"parent-{index}" for index in range(4))

    def fail(_point: C0B5Checkpoint) -> None:
        raise RuntimeError("injected initializer failure")

    with pytest.raises(RuntimeError, match="injected"):
        C0B5Checkpoint.create(
            tmp_path, run_id, header=header(run_id), parent_paths=parents,
            initializer=fail)

    assert not (tmp_path / "runs" / run_id).exists()
    assert not list((tmp_path / "runs").glob(".c0b5-initializing-*"))


def test_post_publish_fsync_failure_removes_published_child(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    run_id = "c0b5-20260812-130000-" + "e" * 24
    parents = tuple(tmp_path / f"parent-{index}" for index in range(4))
...
def test_terminal_state_and_artifact_owner_are_immutable(tmp_path: Path) -> None:
    ...
    with pytest.raises(C0B5CheckpointError, match="immutable"):

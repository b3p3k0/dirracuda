"""Focused offline tests for the frozen C0B-5 public planner."""
...
from scripts.analyst_benchmark.c0b5_plan import (
    C0B5PlanError, LANE_ORDER, build_master_plan, build_request_resolver,
...
def test_seed20260811_and_c0b5_domains_are_fresh(planning_inputs) -> None:
    _corpus, master = planning_inputs
    lanes = _lanes(master)
    assert {row["phase"] for row in lanes[1]["work"]} == {"F_SEED_20260811"}
    assert {row["plan_key"] for row in lanes[1]["work"]} == {"F_SEED_20260811"}
...
def test_controls_are_owned_only_by_seed20260804(planning_inputs) -> None:
    corpus, master = planning_inputs
    controls = master["control_plan"]
    assert controls["context"] | {} == controls["context"]
    assert controls["context"]["lane_id"] == "F72_20260804"
    assert controls["context"]["purpose"] == "c0b5_stage_f_candidate_context"
    assert controls["context"]["trigger_rule"] == \
        "first_bounded_http_terminal_seed20260804"
    assert controls["cancellation"]["seed"] == 20260804
    assert controls["health"]["seed"] == 20260804
    assert all(row["lane_id"] == "F72_20260804" for row in controls.values())
...
def test_request_resolver_rederives_seed20260811_payload(planning_inputs) -> None:
    corpus, master = planning_inputs
    lane = lane_from_master(
        master, "F72_20260811", corpus=corpus, run_nonce_key=KEY)
    work = lane["work"][0]
    ...
    assert spec.payload["options"]["seed"] == 20260811
...
def test_master_tampering_fails_closed(planning_inputs, mutation: str) -> None:
    ...
    if mutation == "seed":
        ...
        changed["lane_plans"][0]["payload"]["work"][0]["nonce"] = \
            "FENCE_" + "A" * 32
    with pytest.raises((C0B5PlanError, RuntimeError), match="parent|differs|identity"):

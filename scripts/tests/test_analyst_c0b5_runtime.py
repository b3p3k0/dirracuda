"""Offline scheduler and recovery tests for the frozen C0B-5 runtime."""
...
from scripts.analyst_benchmark import c0b5_runtime as runtime
...
def _header() -> dict[str, str]:
    return {
        ...
        "run_id": "c0b5-20260811-120000-0123456789abcdef01234567",
...
def test_runtime_event_reconciliation_closes_crash_gap_once() -> None:
    ...
    resolved = {"work": work, "lane": {"lane_id": "F72_20260804"}}
    ...
    assert all(row["version"] == "c0b5-runtime-event-v1" for row in point.events)


def test_reconciliation_skips_exact_c0b5_preflight_owner() -> None:
    point = FakePoint()
    point.attempts.append({
        "attempt_id": _hash("preflight-attempt"),
        "owner_id": stable_hash({"c0b5_preflight": "version", "invocation": 4}),
...
def test_context_crash_recovery_reuses_valid_response_without_http() -> None:
    ...
    assert evidence["version"] == "c0b5-context-evidence-v1"
    assert evidence["lane_id"] == "F72_20260804"
...
def test_existing_context_rederives_trigger_from_first_answered_attempt() -> None:
    ...
    point.artifacts[("lane_plan", "F72_20260804")] = {
        "lane_id": "F72_20260804", "work": [first, second]}
...
def test_planned_cancellation_waits_then_health_checks() -> None:
    ...
    assert evidence["version"] == "c0b5-cancellation-health-evidence-v1"
...
def test_activation_partition_and_cursor_census_are_exact() -> None:
    ...
    activation = point.artifacts[("plan_activation", "F72_20260804")]
    ...
    cursor = point.artifacts[("cursor_transition", "F72_20260804")]
    assert cursor["from_lane_id"] == "F72_20260804"
    assert cursor["to_lane_id"] == "F72_20260811"


@pytest.mark.parametrize("lane_id,reason", [
    ("F72_20260804", "seed20260804_no_qualifier"),
...
def test_quality_failure_reason_owns_exact_stop_point(
    ...
    second_hash = _hash("second") if lane_id == "F72_20260811" else None
    backed_up = []
    ...
    assert result["lane_aggregate_sha256s"]["f72_seed20260804_sha256"] == first_hash
    assert (result["lane_aggregate_sha256s"]["f72_seed20260811_sha256"]
            == second_hash)
...
def test_failure_terminal_owns_evidence_and_no_completion(monkeypatch) -> None:
    ...
    result = runtime._finish_failure(
        ...
        lane_id="F72_20260811", plan_sha256=_hash("plan"))
...
def _aggregate(lane_id, *, passed=True, failures=()):
    return {
        ...
        "cancellation_health_evidence_sha256": (
            _hash("cancel-health") if lane_id == "F72_20260804" else None),
    }
...
def test_serial_scheduler_requires_both_explicit_boundary_resumes(
    ...
    monkeypatch.setattr(runtime.C0B5Checkpoint, "open",
    ...
    assert order == ["preflight", "F72_20260804"]
    with pytest.raises(runtime.C0B5RuntimeError,
    ...
    assert order == [
        "preflight", "F72_20260804", "preflight", "F72_20260811"]
...
@pytest.mark.parametrize("first,second,reason", [
    (_aggregate("F72_20260804", passed=False,
                failures=("negative_false_positive_above_2",)),
     None, "seed20260804_no_qualifier"),
    (_aggregate("F72_20260804"),
...
def test_existing_failed_f_lane_stops_before_any_new_dispatch(
    ...
    point.artifacts[("lane_aggregate", "F72_20260804")] = first
    if second is not None:
        point.artifacts[("lane_aggregate", "F72_20260811")] = second
    backed_up = []
    monkeypatch.setattr(runtime, "ensure_backup_receipt",
                        lambda *_args, **_kwargs: backed_up.append(True))
...
def test_one_child_and_staged_child_both_fail_closed(tmp_path) -> None:
    runs = tmp_path / "runs"
    runs.mkdir()
    child = runs / "c0b5-20260812-120000-0123456789abcdef01234567"
    child.mkdir()
    with pytest.raises(runtime.C0B5RuntimeError, match="already owned"):
        runtime._assert_child_allowance(tmp_path)
    child.rmdir()
    (runs / ".c0b5-initializing-review-required").mkdir()
    with pytest.raises(runtime.C0B5RuntimeError, match="requires review"):
        runtime._assert_child_allowance(tmp_path)
...
def test_terminal_verify_returns_independently_replayed_public_summary(
    ...
    summary = {"version": "c0b5-public-summary-v1", "terminal": "INCONCLUSIVE"}
    ...
    monkeypatch.setattr(runtime, "verify_c0b5_terminal_readonly",

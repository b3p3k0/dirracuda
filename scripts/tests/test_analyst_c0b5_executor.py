"""Offline persistence-boundary tests for the C0B-5 executor."""
...
from scripts.analyst_benchmark.c0b5_executor import (
    C0B5ExecutorError,
...
def test_runtime_event_is_owned_by_c0b5() -> None:
    point = Point()
    value = runtime_event(
        point, event="DISPATCHING", lane_id="F72_20260804",
        attempt_id="c" * 64, work=_work(), occurred_at=1.0)
    assert value["version"] == "c0b5-runtime-event-v1"
...
def test_finish_persists_attempt_before_event() -> None:
    ...
    persist_scored_finish(
        point, "F72_20260804", _work(), request, finish)
...
def test_reconcile_is_idempotent_and_rejects_foreign_owner() -> None:
    ...
    resolver = SimpleNamespace(
        ...
        prepared=SimpleNamespace(resolve_work=lambda _owner: {
            "work": _work(), "lane": {"lane_id": "F72_20260804"}}))
    ...
    point.events.clear()
    with pytest.raises(C0B5ExecutorError, match="outside"):
        reconcile_runtime_events(point, resolver)

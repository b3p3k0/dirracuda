"""Offline command-boundary tests for C0B-5."""
...
from scripts.analyst_benchmark import c0b5_cli, leakscan


def test_package_entrypoint_dispatches_c0b5(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[list[str]] = []
    monkeypatch.setattr(c0b5_cli, "main", lambda args: calls.append(list(args)) or 7)
    assert package_main.main(["c0b5", "status", "--run-id", "run"]) == 7
...
def test_live_confirmation_precedes_runtime_import(
    ...
    module = "scripts.analyst_benchmark.c0b5_runtime"
    monkeypatch.delitem(sys.modules, module, raising=False)
    assert c0b5_cli.main([command, "--run-id", "run"]) == 2
...
def test_create_uses_isolated_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(sys.modules, "scripts.analyst_benchmark.c0b5_runtime", fake)
    assert c0b5_cli.main(["create"]) == 0
    assert calls == ["create"]


def test_leak_scan_uses_exact_c0b5_scope(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[dict[str, object]] = []
    monkeypatch.setattr(leakscan, "run", lambda **kw: calls.append(kw) or 0)
    assert c0b5_cli.main([
    ...
    assert calls == [{
        ...
        "protocol_id": c0b5_cli.BENCHMARK_PROTOCOL_ID,
    }]
...
def test_verify_exit_and_errors_are_redacted(
    ...
    monkeypatch.setitem(sys.modules, "scripts.analyst_benchmark.c0b5_runtime", fake)
    assert c0b5_cli.main(["verify", "--run-id", "child"]) == 4
    ...
    assert c0b5_cli.main(["status", "--run-id", "child"]) == 4

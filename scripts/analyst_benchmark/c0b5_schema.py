"""Strict, no-coercion artifact contracts for C0B-5.
...
PolicyId: TypeAlias = Literal["c0b5-assistive-bounded-fp-v1"]
PolicySha256: TypeAlias = Literal[
    "0af37d83b05e03e4cd336719587d6e98d49aeaec943edeadce2e3df35651b1f7"]
LaneId: TypeAlias = Literal["F72_20260804", "F72_20260811", "C44_1"]
F72LaneId: TypeAlias = Literal["F72_20260804", "F72_20260811"]
Category: TypeAlias = Literal["pii", "financial", "contact", "demographic"]
Component: TypeAlias = Literal[
    "C44_RERUN", "D50_CONFIRMATION", "F72_SEED20260804", "F72_SEED20260811"]
...
QUALITY_REASON_BY_TERMINAL = {
    ...
    "INCONCLUSIVE": (
        "seed20260804_no_qualifier", "seed20260804_control_gate_failed",
        "seed20260811_no_qualifier", "complete_corpus_acceptance_failed",
    ),
}
...
LANE_SEEDS = {"F72_20260804": 20260804, "F72_20260811": 20260811, "C44_1": 1}
LANE_PHASES = {
    "F72_20260804": "F_SEED_20260804",
...
class RunHeader(PolicyBoundStrictModel):
    version: Literal["c0b5-run-header-v1"]
...
def c0b5_activation_group_id(candidate_id: str, plan_key: str) -> str:
    return sha256_json({"candidate_id": candidate_id,
                        "domain": "c0b5-stage-f-group-v1", "plan_key": plan_key})


def c0b5_cell_id(*, candidate_id: str, phase: str, seed: int) -> str:
    return sha256_json({
        "budget_stage": "F", "candidate_id": candidate_id, "chunk_chars": 8000,
        "domain": "c0b5-public-cell-v1", "num_ctx": 8192,
        "num_predict": 1024, "overlap": 256, "phase": phase, "seed": seed,
    })


def c0b5_work_id(*, cell_id: str, chunk_index: int, chunk_sha256: str,
    ...
    return sha256_json({
        ...
        "domain": "c0b5-public-work-v1", "nonce": nonce, "plan_key": plan_key,
        "request_sha256": request_sha256, "view_id": view_id,
    })


class C0B5PublicWork(StrictModel):
    stage: Literal["F"]; phase: Literal["F_SEED_20260804", "F_SEED_20260811", "F_ACCEPTANCE"]
    plan_key: Literal["F_SEED_20260804", "F_SEED_20260811", "F_ACCEPTANCE"]
    ...
    seed: Literal[1, 20260804, 20260811]; chunk_index: int = Field(ge=0)
    chunk_sha256: Sha256; nonce: Nonce; prompt_sha256: Sha256; request_sha256: Sha256

    @model_validator(mode="after")
    def exact_identity(self) -> "C0B5PublicWork":
        ...
        if lane != "C44_1" and self.activation_group_id != c0b5_activation_group_id(
                self.candidate_id, self.plan_key):
            raise ValueError("work activation group differs from C0B-5 identity")
        if self.cell_id != c0b5_cell_id(
                candidate_id=self.candidate_id, phase=self.phase, seed=self.seed):
            raise ValueError("work cell differs from C0B-5 identity")
        if self.work_id != c0b5_work_id(
            ...
            raise ValueError("work ID differs from C0B-5 identity")
        return self
...
class DedupEvidence(PolicyBoundStrictModel):
    version: Literal["c0b5-dedup-evidence-v1"]
...
class C0B5ChunkRow(ChunkRow):
    ...
    @model_validator(mode="after")
    def exact_outcome(self) -> "C0B5ChunkRow":
...
class C0B5DocumentRow(DocumentRow):
    chunks: list[C0B5ChunkRow] = Field(min_length=1)
    redundant_rows: int = Field(ge=0); affected_work_ids: list[Sha256]
    normalized_duplicate_chunks: int = Field(ge=0); affected_document: bool

    @model_validator(mode="after")
    def exact_recovery(self) -> "C0B5DocumentRow":
...
class RetainedMetrics(StrictModel):
    documents: list[C0B5DocumentRow]
...
class LanePlan(PolicyBoundStrictModel):
    version: Literal["c0b5-lane-plan-v1"]
    lane_id: F72LaneId; seed: Literal[20260804, 20260811]; candidate: Candidate
    parent_evidence: ParentBinding; work: list[C0B5PublicWork] = Field(min_length=92, max_length=92)
    plan_sha256: Sha256
...
class AcceptancePlan(PolicyBoundStrictModel):
    version: Literal["c0b5-acceptance-plan-v1"]
    lane_id: Literal["C44_1"]; seed: Literal[1]; candidate: Candidate
    parent_evidence: ParentBinding; work: list[C0B5PublicWork] = Field(min_length=44, max_length=44)
    plan_sha256: Sha256
...
class PlanActivation(PolicyBoundStrictModel):
    version: Literal["c0b5-plan-activation-v1"]
...
class CursorTransition(PolicyBoundStrictModel):
    version: Literal["c0b5-cursor-transition-v1"]
    ...
    def exact_transition(self) -> "CursorTransition":
        if (self.from_lane_id, self.to_lane_id) not in {
                ("F72_20260804", "F72_20260811"), ("F72_20260811", "C44_1")}:
...
class RuntimeEvent(PolicyBoundStrictModel):
    version: Literal["c0b5-runtime-event-v1"]
...
class ContextControl(PolicyBoundStrictModel):
    version: Literal["c0b5-context-control-v1"]
    control_id: Sha256; kind: Literal["context_probe"]
    lane_id: Literal["F72_20260804"]; purpose: Literal["c0b5_stage_f_candidate_context"]
    ...
    trigger_rule: Literal["first_bounded_http_terminal_seed20260804"]
    payload_sha256: Sha256


class ContextEvidence(PolicyBoundStrictModel):
    version: Literal["c0b5-context-evidence-v1"]
    control_id: Sha256; lane_id: Literal["F72_20260804"]
    purpose: Literal["c0b5_stage_f_candidate_context"]; candidate_id: Sha256
    model: Literal["qwen3.6:27b"]
    model_digest: Literal[
        "a50eda8ed977ab48a12431878896b27ffd5cef552c17af3317d9623b939a7f1e"]
...
class CancellationControl(PolicyBoundStrictModel):
    version: Literal["c0b5-cancellation-control-v1"]
    control_id: Sha256; kind: Literal["cancellation_probe"]
    lane_id: Literal["F72_20260804"]; candidate_id: Sha256; seed: Literal[20260804]
...
class HealthControl(PolicyBoundStrictModel):
    version: Literal["c0b5-health-control-v1"]
...
class MasterPlan(PolicyBoundStrictModel):
    version: Literal["c0b5-master-plan-v1"]
    ...
    def exact_tree(self) -> "MasterPlan":
        expected = ["F72_20260804", "F72_20260811", "C44_1"]
...
class CancellationHealthEvidence(PolicyBoundStrictModel):
    version: Literal["c0b5-cancellation-health-evidence-v1"]
    lane_id: Literal["F72_20260804"]; candidate_id: Sha256; prompt_sha256: Sha256
...
class LaneAggregate(PolicyBoundStrictModel):
    version: Literal["c0b5-lane-aggregate-v1"]
    lane_id: F72LaneId; seed: Literal[20260804, 20260811]; lane_plan_sha256: Sha256
    ...
    def exact_gate(self) -> "LaneAggregate":
        ...
        first = self.lane_id == "F72_20260804" and self.seed == 20260804
        second = self.lane_id == "F72_20260811" and self.seed == 20260811
...
class C44ScoredAggregate(PolicyBoundStrictModel):
    version: Literal["c0b5-c44-scored-v1"]
...
class ComponentHashes(StrictModel):
    ...
    f72_seed20260804_aggregate_sha256: Sha256


class AcceptanceAggregate(PolicyBoundStrictModel):
    version: Literal["c0b5-acceptance-aggregate-v1"]
...
class LaneAggregateHashes(StrictModel):
    f72_seed20260804_sha256: Sha256
    f72_seed20260811_sha256: Sha256 | None
    c44_scored_sha256: Sha256 | None


class Result(PolicyBoundStrictModel):
    version: Literal["c0b5-result-v1"]
    ...
    def exact_terminal(self) -> "Result":
        ...
        if (confirmed != (self.selection is not None)
                ...
                or second != (hashes.f72_seed20260811_sha256 is not None)
...
class Completion(PolicyBoundStrictModel):
    version: Literal["c0b5-completion-v1"]
...
class FailureEvidence(PolicyBoundStrictModel):
    version: Literal["c0b5-failure-evidence-v1"]
    terminal: FailureTerminal; reason: str; lane_id: LaneId | None
...
class FailureResult(PolicyBoundStrictModel):
    version: Literal["c0b5-failure-v1"]
    terminal: FailureTerminal; reason: str; evidence_sha256: Sha256
...
class BackupAnchor(PolicyBoundStrictModel):
    version: Literal["c0b5-backup-anchor-v1"]
...
class BackupReceipt(PolicyBoundStrictModel):
    version: Literal["c0b5-backup-receipt-v1"]
...
class PublicSummary(PolicyBoundStrictModel):
    version: Literal["c0b5-public-summary-v1"]
    ...
    def exact_summary(self) -> "PublicSummary":
        components = ("C44_RERUN", "D50_CONFIRMATION",
                      "F72_SEED20260804", "F72_SEED20260811")
        rows = self.false_positive_documents
        second = self.lane_aggregate_sha256s.f72_seed20260811_sha256 is not None
        final = self.acceptance_aggregate_sha256 is not None
        expected_presence = {
            "C44_RERUN": final, "D50_CONFIRMATION": True,
            "F72_SEED20260804": True, "F72_SEED20260811": second,
        }
        complete = self.reason in {
            "complete_public_acceptance_passed", "complete_corpus_acceptance_failed"}
        ...
        if final:
            included = ("C44_RERUN", "D50_CONFIRMATION", "F72_SEED20260804")
            expected_total = sum(
...
_VERSION_MODELS: dict[str, type[BaseModel]] = {
    "c0b5-run-header-v1": RunHeader, "c0b5-master-plan-v1": MasterPlan,
    "c0b5-lane-plan-v1": LanePlan, "c0b5-acceptance-plan-v1": AcceptancePlan,
    "c0b5-plan-activation-v1": PlanActivation,
    "c0b5-cursor-transition-v1": CursorTransition,
    "c0b5-runtime-event-v1": RuntimeEvent,
    "c0b5-context-control-v1": ContextControl,
    "c0b5-context-evidence-v1": ContextEvidence,
    "c0b5-cancellation-control-v1": CancellationControl,
    "c0b5-health-control-v1": HealthControl,
    "c0b5-cancellation-health-evidence-v1": CancellationHealthEvidence,
    "c0b5-dedup-evidence-v1": DedupEvidence,
    "c0b5-lane-aggregate-v1": LaneAggregate,
    "c0b5-c44-scored-v1": C44ScoredAggregate,
    "c0b5-acceptance-aggregate-v1": AcceptanceAggregate,
    "c0b5-result-v1": Result, "c0b5-completion-v1": Completion,
    "c0b5-failure-evidence-v1": FailureEvidence,
    "c0b5-failure-v1": FailureResult, "c0b5-backup-anchor-v1": BackupAnchor,
    "c0b5-backup-receipt-v1": BackupReceipt,
}


def validate_artifact(value: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError("C0B-5 artifact must be a mapping")
    require_current_payload(value)
    model = _VERSION_MODELS.get(value.get("version"))
    if model is None:
        raise ValueError("unknown C0B-5 artifact version")

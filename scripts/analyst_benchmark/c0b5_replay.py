"""Independent C0B-3 D50, C0B-4 parent, and C0B-5 terminal replay.
...
from .c0b5_checkpoint import sha256_json as c0b5_sha256_json, validate_run_lineage
from .c0b5_lineage import FROZEN_EXECUTION_PARENT, FROZEN_OBSERVED_C0B4
from .c0b5_plan import (LANE_ORDER as C0B5_LANE_ORDER, build_request_resolver, validate_master_plan)
from .c0b5_schema import C0B5ChunkRow, validate_artifact
from .c0b5_scoring import (
...
class C0B5ReplayError(RuntimeError):
...
class C0B5ReplayFacts:
    ...
    def without_receipt(self) -> "C0B5ReplayFacts":
        return replace(
            self, backup_anchor_sha256=None, backup_snapshot_sha256=None,
            backup_snapshot_size=None, backup_receipt_sha256=None)
...
def _exact_json(raw: Any, digest: Any, label: str) -> dict[str, Any]:
    if type(raw) is not str or type(digest) is not str:
        raise C0B5ReplayError(f"{label} row has invalid storage types")
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise C0B5ReplayError(f"{label} is invalid JSON") from exc
    if type(value) is not dict or _canonical(value) != raw \
            or _sha256_json(value) != digest:
        raise C0B5ReplayError(f"{label} is noncanonical or changed")
    return value
...
def _d50_template_family(document_id: str) -> str:
    ...
    raise C0B5ReplayError("D50 negative document ID has no frozen template family")
...
def _c3_d50_attempts(
    ...
    if len(registry) != len(work) or {row[0] for row in registry} != set(work):
        raise C0B5ReplayError("C0B-3 D50 registry differs from its frozen plan")
    ...
    for row in rows:
        ...
        if (row[2] is not None or row[3] != "D" or type(row[4]) is not int
                or not conn.execute(
                    ...
                    (row[4],)).fetchone()):
            raise C0B5ReplayError("C0B-3 D50 attempt ownership changed")
        try:
            metadata = json.loads(row[10]) if row[10] is not None else {}
        except (TypeError, json.JSONDecodeError) as exc:
            raise C0B5ReplayError("C0B-3 D50 metadata is invalid JSON") from exc
        if type(metadata) is not dict or _canonical(metadata) != row[10]:
            raise C0B5ReplayError("C0B-3 D50 metadata is noncanonical")
        ...
        if answered:
            ...
            try:
                ...
                raise C0B5ReplayError("C0B-3 D50 answer is invalid JSON") from exc
            if (type(response) is not str
                ...
                raise C0B5ReplayError("C0B-3 D50 answer byte identity changed")
        evidence = {
        ...
        try:
            ...
            raise C0B5ReplayError("C0B-3 D50 bounded attempt is invalid") from exc
    ...
    for work_id, attempts in grouped.items():
        ...
        if (len(state) != 1 or state[0][2] != work[work_id]["request_sha256"]
            ...
            raise C0B5ReplayError("C0B-3 D50 work state differs from attempts")
    return grouped
...
def replay_c0b3_d50_connection(conn: sqlite3.Connection) -> C0B3D50ReplayFacts:
    ...
    if len(manifest_rows) != 1:
        raise C0B5ReplayError("C0B-3 master manifest census changed")
    ...
    if (type(manifest_raw) is not str or hashlib.sha256(manifest_raw.encode()).hexdigest()
        ...
        raise C0B5ReplayError("C0B-3 master manifest identity changed")
    try:
        corpus = load_d50(
            manifest_raw, master_manifest_sha256=manifest_sha,
            manifest_path=goldset.MANIFEST)
    except Exception as exc:
        raise C0B5ReplayError("C0B-3 D50 public corpus changed") from exc
    plan_rows = conn.execute(
        "SELECT plan_hash,plan_json FROM phase_plans "
        "WHERE plan_key='D4_CONFIRMATION'").fetchall()
    if len(plan_rows) != 1:
        raise C0B5ReplayError("C0B-3 D50 plan census changed")
    ...
    if (plan.get("phase") != "D4" or len(plan.get("work", ())) != 66
        ...
        raise C0B5ReplayError("C0B-3 D50 plan shape changed")
    activation = conn.execute(
        "SELECT activation_hash,activation_json FROM plan_activations "
        "WHERE plan_key='D4_CONFIRMATION'").fetchall()
    if len(activation) != 1:
        raise C0B5ReplayError("C0B-3 D50 activation census changed")
    ...
    if (activated.get("state") != "ACTIVATED"
        ...
        raise C0B5ReplayError("C0B-3 D50 activation differs from its plan")
    ...
    if len(controls) != 1 or controls[0][2] != "COMPLETE":
        raise C0B5ReplayError("C0B-3 D50 context-control census changed")
    ...
    try:
        ...
        raise C0B5ReplayError("C0B-3 D50 raw attempts do not replay") from exc
    aggregate_rows = conn.execute(
        "SELECT plan_hash,aggregate_hash,aggregate_json FROM phase_aggregates "
        "WHERE plan_key='D4_CONFIRMATION'").fetchall()
    if len(aggregate_rows) != 1:
        raise C0B5ReplayError("C0B-3 D50 aggregate census changed")
    stored = _exact_json(
    ...
    if (aggregate_rows[0][0] != plan_rows[0][0]
        ...
        raise C0B5ReplayError("C0B-3 D50 aggregate does not independently rederive")
    ...
    if len(decision_rows) != 1:
        raise C0B5ReplayError("C0B-3 final-D decision census changed")
    ...
    if (type(decision) is not dict or _canonical(decision) != decision_row[4]
        ...
        raise C0B5ReplayError("C0B-3 final-D decision identity changed")
    ...
    if (len(public_rows) != quality["negative_false_positive_documents"]
        ...
        raise C0B5ReplayError("C0B-3 D50 negative-row census does not replay")
...
def _load_header(conn: sqlite3.Connection) -> tuple[dict[str, Any], str]:
    ...
    if len(rows) != 1:
        raise C0B5ReplayError("C0B-4 run-header census changed")
    ...
    if set(header) != HEADER_KEYS or any(header.get(key) != value
        ...
        raise C0B5ReplayError("C0B-4 run header is mixed or differs from frozen literals")
    if rows[0][1] != observed["run_header_sha256"]:
        raise C0B5ReplayError("C0B-4 run-header digest differs from frozen evidence")
...
def _load_artifacts(
    ...
    if {(row[0], row[1]) for row in rows} != EXPECTED_ARTIFACT_OWNERS \
        ...
        raise C0B5ReplayError("C0B-4 artifact owner census changed")
    ...
    for kind, owner, digest, raw in rows:
        ...
        if any(value.get(key) != header[key] for key in (
            ...
            raise C0B5ReplayError("C0B-4 artifact has a mixed policy identity")
    ...
    if any(artifacts[key][1] != digest for key, digest in pins.items()):
        raise C0B5ReplayError("C0B-4 frozen artifact digest changed")
    return artifacts
...
def _load_corpus(header: Mapping[str, Any]) -> PublicCorpus:
    ...
    if manifest.sha256 != header["master_manifest_sha256"]:
        raise C0B5ReplayError("public corpus manifest differs from C0B-4")
...
def _source_chunk(work: Mapping[str, Any], corpus: PublicCorpus) -> tuple[str, int]:
    ...
    if document is None or document.document_sha256 != work["document_sha256"]:
        raise C0B5ReplayError("C0B-4 work differs from its public document")
    source, view_id = document.source_for(
        work["chunk_chars"], derived=work["view_id"] is not None)
    if view_id != work["view_id"]:
        raise C0B5ReplayError("C0B-4 work has the wrong public document view")
...
def _plans(
    ...
    if (set(master) != {"version", "policy_id", "policy_sha256", "protocol_sha256",
        ...
        raise C0B5ReplayError("C0B-4 master plan shape or lineage changed")
    ...
    for lane_id, envelope in zip(LANE_ORDER, envelopes, strict=True):
        if type(envelope) is not dict or set(envelope) != {"plan_sha256", "payload"}:
            raise C0B5ReplayError("C0B-4 lane envelope shape changed")
        ...
        if type(plan) is not dict or plan != stored or plan.get("lane_id") != lane_id:
            raise C0B5ReplayError("C0B-4 lane differs from master-plan envelope")
        body = dict(plan)
        ...
        if plan_sha != _sha256_json(body) or envelope["plan_sha256"] != plan_sha \
            ...
            raise C0B5ReplayError("C0B-4 lane self-digest changed")
        ...
        if (set(plan) != {"version", "policy_id", "policy_sha256", "protocol_sha256",
            ...
            raise C0B5ReplayError("C0B-4 lane plan contract changed")
        try:
            work = [PublicWork.model_validate(row, strict=True).model_dump(mode="json")
                    for row in plan["work"]]
        except (TypeError, ValueError) as exc:
            raise C0B5ReplayError("C0B-4 work row is invalid") from exc
        ...
        if (len({row["work_id"] for row in work}) != count
                ...
                or any(row["seed"] != seed or row["plan_key"] != plan_key
                       for row in work)):
            raise C0B5ReplayError("C0B-4 lane work census/order changed")
        for row in work:
...
def _activation_and_inactive_census(
    ...
    if (set(activation) != {"version", "policy_id", "policy_sha256", "protocol_sha256",
        ...
        raise C0B5ReplayError("C0B-4 initial activation does not rederive")
    ...
    if later != FROZEN_OBSERVED_C0B4["inactive_lane_census"]:
        raise C0B5ReplayError("C0B-4 inactive-lane census changed")
    return later
...
def _attempt_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    if len(rows) != 96 or len({row["attempt_id"] for row in rows}) != 96:
        raise C0B5ReplayError("C0B-4 attempt census changed")
    ...
    if invocation_rows != [(1,)] or limits != {
            ...
            "preflight_control": 33, "transport_orphan": 30}:
        raise C0B5ReplayError("C0B-4 invocation or ledger contract changed")
    for row in rows:
        ...
        if history != [("DISPATCHING", None), (row["state"], row["payload_json"])] \
            ...
            raise C0B5ReplayError("C0B-4 attempt history changed")
        if row["payload_json"] is None:
            raise C0B5ReplayError("C0B-4 terminal attempt lacks payload evidence")
        try:
            payload = json.loads(row["payload_json"])
        except json.JSONDecodeError as exc:
            raise C0B5ReplayError("C0B-4 attempt payload is invalid JSON") from exc
        if _canonical(payload) != row["payload_json"]:
            raise C0B5ReplayError("C0B-4 attempt payload is noncanonical")
        row["payload"] = payload
    return rows
...
def _validate_control_attempts(
    ...
    if len(controls) != 4 or any(row["state"] != "RAW_VALID" for row in controls):
        raise C0B5ReplayError("C0B-4 preflight/context attempt census changed")
    ...
    if {row["owner_id"]: row["request_sha256"] for row in controls} != expected:
        raise C0B5ReplayError("C0B-4 preflight/context identities changed")
    for row in controls:
        if row["attempt_id"] != attempt_id(f"control:{row['owner_id']}", 1):
            raise C0B5ReplayError("C0B-4 control attempt identity changed")
...
def _scored_chunks(
    ...
    if (len(scored) != 92 or {row["owner_id"] for row in scored} != set(work_by_id)
        ...
        raise C0B5ReplayError("C0B-4 scored-attempt ownership changed")
    ...
    for attempt in scored:
        ...
        if (attempt["request_sha256"] != work["request_sha256"]
            ...
            raise C0B5ReplayError("C0B-4 scored attempt differs from its plan")
        ...
        if type(payload) is not dict or set(payload) != {"answered", "response", "metadata"} \
            ...
            raise C0B5ReplayError("C0B-4 scored payload shape changed")
        ...
        if (assessment.final_outcome != "RAW_VALID" or not assessment.eventual_valid
            ...
            raise C0B5ReplayError("C0B-4 answer assessment does not replay")
        ...
        if (type(prompt_count) is not int or prompt_count < 0
            ...
            raise C0B5ReplayError("C0B-4 response metadata is incomplete")
        retained_value = assessment.retained_value
        if retained_value is None:
            raise C0B5ReplayError("C0B-4 raw-valid answer lacks retained value")
        ...
        for index, finding in enumerate(retained_value["findings"]):
            ...
            if not grounding.grounded or grounding.canonical_offset is None:
                raise C0B5ReplayError("C0B-4 retained finding is ungrounded")
...
def _document_rows(
    ...
    if list(grouped) != list(corpus.f_order):
        raise C0B5ReplayError("C0B-4 F72 document order changed")
...
def _verify_context(
    ...
    if key not in artifacts:
        raise C0B5ReplayError("C0B-4 context evidence is absent")
    ...
    if (evidence.get("version") != "c0b4-context-evidence-v1"
        ...
        raise C0B5ReplayError("C0B-4 context evidence does not rederive")
    return digest
...
def _terminal_and_receipt(
    ...
    if (states != [("INCONCLUSIVE",)]
        ...
        raise C0B5ReplayError("C0B-4 terminal result/completion does not rederive")
    ...
    if not require_receipt:
        if receipts:
            raise C0B5ReplayError("pre-receipt C0B-4 snapshot contains a receipt")
        return result_sha, completion_sha, "seed17_no_qualifier", None, None, None, None
    if len(receipts) != 1:
        raise C0B5ReplayError("C0B-4 terminal receipt census changed")
    ...
    if (_sha256_json(anchor, omit="anchor_sha256") != anchor_sha
        ...
        raise C0B5ReplayError("C0B-4 backup anchor/receipt does not rederive")
...
def replay_c0b4_connection(
    ...
    if _canonical(derived) != _canonical(stored_aggregate):
        raise C0B5ReplayError("C0B-4 F72 aggregate does not independently rederive")
    ...
    if not all(comparisons.values()):
        raise C0B5ReplayError("C0B-4 replay differs from frozen observed literals")
    return facts
...
def verify_observed_c0b4_readonly(
    ...
    from .c0b5_lineage import _PinnedSQLite
    with _PinnedSQLite(checkpoint, trusted_root) as source, \
        ...
        if (source.sha256 != FROZEN_OBSERVED_C0B4["checkpoint_sha256"]
            ...
            raise C0B5ReplayError("C0B-4 checkpoint or snapshot bytes changed")
        ...
        if facts.without_receipt() != backup_facts.without_receipt():
            raise C0B5ReplayError("C0B-4 checkpoint/snapshot replay mismatch")
        if (facts.backup_snapshot_sha256 != backup.sha256
                or facts.backup_snapshot_size != backup.identity().size):
            raise C0B5ReplayError("C0B-4 receipt snapshot binding changed")
        return facts


def _c5_artifacts(conn: sqlite3.Connection,
                  ) -> dict[tuple[str, str], tuple[dict[str, Any], str]]:
    ...
    for kind, owner, digest, raw in conn.execute(
        ...
        value = _exact_json(
            ...
            digest, f"C0B-5 {kind}/{owner}")
        try:
            normalized = validate_artifact(value)
        except (TypeError, ValueError) as exc:
            raise C0B5ReplayError("C0B-5 artifact violates its closed schema") from exc
        if normalized != value or (kind, owner) in values:
            raise C0B5ReplayError("C0B-5 artifact is noncanonical or duplicated")
        values[(kind, owner)] = value, digest
    return values


def _c5_header(conn: sqlite3.Connection) -> tuple[dict[str, Any], str]:
    rows = conn.execute("SELECT json,sha256 FROM run_header ORDER BY id").fetchall()
    if len(rows) != 1:
        raise C0B5ReplayError("C0B-5 run-header census changed")
    raw, digest = rows[0]
    header = _exact_json(
        raw.decode("utf-8") if type(raw) is bytes else raw,
        digest, "C0B-5 run header")
    try:
        normalized = validate_artifact(header)
    except (TypeError, ValueError) as exc:
        raise C0B5ReplayError("C0B-5 run header violates its closed schema") from exc
    if normalized != header:
        raise C0B5ReplayError("C0B-5 run header is not exact")
    return header, rows[0][1]


def _c5_scored_evidence(
    ...
    if len(work) != 228:
        raise C0B5ReplayError("C0B-5 master work index changed")
    ...
    for attempt_id_value, owner, call_class, ordinal, request_sha, state, raw in \
        ...
        if (call_class not in {"scored", "schema_retry", "transport_orphan"}
            ...
            raise C0B5ReplayError("C0B-5 scored-attempt ownership changed")
        ...
        if raw_text is not None and (
            ...
            raise C0B5ReplayError("C0B-5 attempt payload is noncanonical")
    ...
    for work_id, rows in attempts.items():
        ...
        for index, row in enumerate(rows, 1):
            ...
            if closed or row["call_class"] != expected_class:
                raise C0B5ReplayError("C0B-5 attempt sequence changed")
            if payload is None or type(payload.get("answered")) is not bool:
                raise C0B5ReplayError("C0B-5 scored payload is incomplete")
            response = payload.get("response")
            if payload["answered"]:
                if type(response) is not str or type(payload.get("metadata")) is not dict:
                    raise C0B5ReplayError("C0B-5 answered payload is malformed")
                ...
                if (row["state"] != expected_state
                    ...
                    raise C0B5ReplayError("C0B-5 raw answer assessment does not replay")
                ...
                raise C0B5ReplayError("C0B-5 unanswered attempt exposes answer fields")
        ...
        if not answered or not closed:
            raise C0B5ReplayError("C0B-5 terminal work has incomplete answer history")
        ...
        if any(type(value) is not int for value in prompts):
            raise C0B5ReplayError("C0B-5 prompt-evaluation evidence is incomplete")
        dedup = None
        dedup_hash = None
        if answer.final_outcome == "NORMALIZED_DUPLICATE":
            key = ("dedup_evidence", work_id)
            if key not in artifacts:
                raise C0B5ReplayError("C0B-5 normalized answer lacks dedup evidence")
            dedup, dedup_hash = artifacts[key]
            expected = {
                "version": "c0b5-dedup-evidence-v1",
            ...
            expected["evidence_sha256"] = c0b5_sha256_json(expected)
            if _canonical(expected) != _canonical(dedup):
                raise C0B5ReplayError("C0B-5 dedup evidence does not rederive")
        elif ("dedup_evidence", work_id) in artifacts:
            raise C0B5ReplayError("C0B-5 raw/invalid answer owns dedup evidence")
        chunk = {
        ...
        try:
            parsed = C0B5ChunkRow.model_validate(
                chunk, strict=True).model_dump(mode="json")
        except (TypeError, ValueError) as exc:
            raise C0B5ReplayError("C0B-5 chunk evidence is contradictory") from exc
        result[work_id] = {"chunk": parsed, "retained_findings": findings,
                           "dedup_evidence": deepcopy(dedup)}
    return result, unknown


def _c5_receipt(conn: sqlite3.Connection, header: Mapping[str, Any], result_sha: str,
    ...
    if not require_receipt:
        if rows:
            raise C0B5ReplayError("pre-receipt C0B-5 snapshot contains a receipt")
        return None, None, None, None
    if len(rows) != 1:
        raise C0B5ReplayError("C0B-5 terminal receipt census changed")
    ...
    receipt = _exact_json(
        ...
        "C0B-5 backup receipt")
    try:
        anchor = validate_artifact(anchor)
        receipt = validate_artifact(receipt)
    except (TypeError, ValueError) as exc:
        raise C0B5ReplayError("C0B-5 receipt violates its closed schema") from exc
    ...
    if (anchor_sha != anchor["anchor_sha256"]
        ...
        raise C0B5ReplayError("C0B-5 receipt chain does not bind its terminal")
    return (anchor_sha, receipt["snapshot_sha256"],
            receipt["snapshot_size_bytes"], receipt_sha)


def _c5_control_evidence(
        conn: sqlite3.Connection, controls: Mapping[str, Any],
        plans: Mapping[str, Mapping[str, Any]],
        artifacts: Mapping[tuple[str, str], tuple[dict[str, Any], str]],
) -> tuple[str, str | None, bool | None]:
    first_work = {row["work_id"]: row for row in plans["F72_20260804"]["work"]}
    rows = []
    ...
    if context_artifact is None or len(context_attempts) != 1 or not answered:
        raise C0B5ReplayError("C0B-5 context evidence census changed")
    ...
    try:
        ...
        raise C0B5ReplayError("C0B-5 context response is malformed") from exc
    trigger = answered[0]
    trigger_work = first_work[trigger[2]]
    expected_context = {
        "version": "c0b5-context-evidence-v1",
        **{name: context_artifact[0][name] for name in (
            "policy_id", "policy_sha256", "protocol_sha256")},
        "control_id": context_id, "lane_id": "F72_20260804",
    ...
    if (context_attempt[3] != "RAW_VALID"
        ...
        raise C0B5ReplayError("C0B-5 context evidence does not independently rederive")
    ...
    if len(cancelled) != 1 or not health_answered:
        raise C0B5ReplayError("C0B-5 cancellation/health attempt census changed")
    ...
    expected_cancel = {
        "version": "c0b5-cancellation-health-evidence-v1",
        **{name: cancellation[0][name] for name in (
            "policy_id", "policy_sha256", "protocol_sha256")},
        "lane_id": "F72_20260804",
    ...
    if (_canonical(cancellation[0]) != _canonical(expected_cancel)
        ...
        raise C0B5ReplayError(
            "C0B-5 cancellation/health evidence does not independently rederive")
    return context_artifact[1], cancellation[1], not reasons


def _c5_failure_terminal(
    ...
    if (failure_row is None or evidence_row is None
        ...
        raise C0B5ReplayError("C0B-5 failure terminal owner census changed")
    ...
    if (failure["terminal"] != state or evidence["terminal"] != state
        ...
        raise C0B5ReplayError("C0B-5 failure facts do not rederive")
    ...
    if ((lane_id is None) != (plan_sha is None)
        ...
        raise C0B5ReplayError("C0B-5 failure lane ownership changed")
    ...
    if control_id is not None and control_id not in control_ids:
        raise C0B5ReplayError("C0B-5 failure names an unknown control")
    ...
    if attempt_value is not None:
        ...
        if len(rows) != 1 or rows[0][1] != state:
            raise C0B5ReplayError("C0B-5 failure attempt does not own its terminal")
        owner = rows[0][0]
        ...
        if (control_id is not None and owner != control_id
            ...
            raise C0B5ReplayError("C0B-5 failure attempt owner changed")
    anchor, snapshot, size, receipt = _c5_receipt(
        conn, header, failure_sha, None, require_receipt=require_receipt)
    return C0B5ReplayFacts(
...
def _c5_schedule(
    ...
    for index, lane_id in enumerate(active):
        ...
        later = C0B5_LANE_ORDER[C0B5_LANE_ORDER.index(lane_id) + 1:]
        ...
        if (activation is None
            ...
            raise C0B5ReplayError("C0B-5 activation chain does not rederive")
        if index:
            ...
            if (transition is None
                ...
                raise C0B5ReplayError("C0B-5 cursor transition does not rederive")
    ...
    if actual != expected_transitions:
        raise C0B5ReplayError("C0B-5 cursor-transition owner census changed")


def replay_c0b5_connection(conn: sqlite3.Connection, *, parent_facts: Any,
                           require_receipt: bool,
) -> C0B5ReplayFacts:
    """Rederive a quality terminal and public view from raw C0B-5 evidence."""
    if not isinstance(conn, sqlite3.Connection) or type(require_receipt) is not bool:
        raise TypeError("read-only SQLite connection and exact receipt flag required")
    try:
        validate_run_lineage(conn)
    except Exception as exc:
        raise C0B5ReplayError("C0B-5 checkpoint lineage is invalid") from exc
    conn.execute("PRAGMA query_only=ON")
    header, header_sha = _c5_header(conn)
    try:
        binding = parent_facts.parent_binding
        d50_facts = parent_facts.c0b3_d50_facts
    except AttributeError as exc:
        raise C0B5ReplayError("verified parent facts are required") from exc
    if header.get("parent_binding") != binding:
        raise C0B5ReplayError("C0B-5 header differs from verified parent binding")
    artifacts = _c5_artifacts(conn)
    master_key = ("master_plan", "master")
    if master_key not in artifacts:
        raise C0B5ReplayError("C0B-5 master plan is absent")
    master, master_sha = artifacts[master_key]
    source = goldset.load(verify=True)
    manifest = build_master_manifest(source)
    if manifest.sha256 != header.get("master_manifest_sha256"):
        raise C0B5ReplayError("C0B-5 public manifest identity changed")
    ...
    if (len(key_rows) != 1 or type(key_rows[0][0]) is not bytes
        ...
        raise C0B5ReplayError("C0B-5 protected nonce key changed")
    try:
        ...
        raise C0B5ReplayError("C0B-5 master plan does not independently rederive") from exc
    ...
    for ordinal in invocation_ordinals:
        control_requests.update({
            stable_hash({"c0b5_preflight": name, "invocation": ordinal}):
    ...
    for attempt_value, owner, request_sha in conn.execute(
        ...
        if (control_requests.get(owner) != request_sha
            ...
            raise C0B5ReplayError("C0B-5 control attempt identity changed")
    context_control_facts = _c5_control_evidence(
    ...
    if result_row is None or completion_row is None:
        raise C0B5ReplayError("C0B-5 quality terminal artifacts are absent")
    ...
    if (result["terminal"] != state
        ...
        raise C0B5ReplayError("C0B-5 result/completion ownership changed")
    active = ["F72_20260804"]
    if result["lane_aggregate_sha256s"]["f72_seed20260811_sha256"] is not None:
        active.append("F72_20260811")
    if result["lane_aggregate_sha256s"]["c44_scored_sha256"] is not None:
        active.append("C44_1")
    derived_lanes: dict[str, dict[str, Any]] = {}
    stored_kind = {"F72_20260804": "lane_aggregate",
                   "F72_20260811": "lane_aggregate", "C44_1": "c44_aggregate"}
    result_key = {"F72_20260804": "f72_seed20260804_sha256",
                  "F72_20260811": "f72_seed20260811_sha256",
                  "C44_1": "c44_scored_sha256"}
    aggregate_hashes = {
        lane_id: result["lane_aggregate_sha256s"][result_key[lane_id]]
        for lane_id in active}
    _c5_schedule(master_sha, plans, artifacts, active, aggregate_hashes)
    for lane_id in active:
        ...
        if len(owned) != len(plans[lane_id]["work"]):
            raise C0B5ReplayError("C0B-5 active lane lacks exact attempt coverage")
        ...
        try:
            ...
            raise C0B5ReplayError("C0B-5 lane attempt evidence does not replay") from exc
        ...
        if (stored is None or _canonical(stored[0]) != _canonical(derived)
            ...
            raise C0B5ReplayError("C0B-5 stored lane aggregate does not rederive")
    ...
    if inactive_work & set(evidence) or any(
        ...
        raise C0B5ReplayError("C0B-5 inactive lane owns execution evidence")
    try:
        ...
        raise C0B5ReplayError("C0B-5 D50 parent component does not replay") from exc
    acceptance = None
    if "C44_1" in active:
        acceptance = build_acceptance_aggregate(
            derived_lanes["C44_1"], d50, derived_lanes["F72_20260804"],
        ...
        if (stored is None or _canonical(stored[0]) != _canonical(acceptance)
            ...
            raise C0B5ReplayError("C0B-5 final acceptance does not rederive")
    if (len(active) == 1 and derived_lanes["F72_20260804"]["passed"]
            or len(active) == 2 and derived_lanes["F72_20260811"]["passed"]):
        raise C0B5ReplayError("C0B-5 terminal stopped before its next required lane")
    expected_reason = (
        "seed20260804_control_gate_failed" if not derived_lanes["F72_20260804"]["passed"]
        and derived_lanes["F72_20260804"]["failure_reasons"] ==
        ["cancellation_health_failure"] else
        "seed20260804_no_qualifier" if not derived_lanes["F72_20260804"]["passed"] else
        "seed20260811_no_qualifier" if "F72_20260811" in derived_lanes
        and not derived_lanes["F72_20260811"]["passed"] else
    ...
    if result["reason"] != expected_reason:
        raise C0B5ReplayError("C0B-5 terminal reason does not follow replayed gates")
    public = build_public_summary(
        run_id=header["run_id"], result=result, completion=completion,
        f72_seed20260804_lane=derived_lanes["F72_20260804"],
        f72_seed20260811_lane=derived_lanes.get("F72_20260811"),
...
def verify_c0b5_terminal_readonly(
        checkpoint: Path, snapshot: Path, *, trusted_root: Path,
        parent_facts: Any,
) -> C0B5ReplayFacts:
    ...
    with _PinnedSQLite(checkpoint, trusted_root) as source, \
            _PinnedSQLite(snapshot, trusted_root) as backup:
        assert source.conn is not None and backup.conn is not None
        facts = replay_c0b5_connection(
            source.conn, parent_facts=parent_facts, require_receipt=True)
        backup_facts = replay_c0b5_connection(
            backup.conn, parent_facts=parent_facts, require_receipt=False)
        if facts.without_receipt() != backup_facts.without_receipt():
            raise C0B5ReplayError("C0B-5 checkpoint/snapshot replay mismatch")
        if (facts.backup_snapshot_sha256 != backup.sha256
                or facts.backup_snapshot_size != backup.identity().size):
            raise C0B5ReplayError("C0B-5 receipt snapshot binding changed")
        return facts

"""Descriptor-pinned C0B-3/C0B-4 parent verification for C0B-5.
...
C0B-4 terminal semantics are replayed independently by :mod:`c0b5_replay`.
...
class C0B5LineageError(RuntimeError):
...
def validate_parent_binding(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(value) is not dict or _canonical(value) != _canonical(FROZEN_PARENT_BINDING):
        raise C0B5LineageError("parent binding differs from frozen C0B-3/C0B-4 evidence")
...
class _PinnedSQLite:
    ...
    def __enter__(self) -> "_PinnedSQLite":
        ...
        if nofollow is None or directory is None:
            raise C0B5LineageError("platform lacks pinned no-follow reads")
        ...
        if any(part in {".", ".."} for part in (*target.parts[1:], *root.parts[1:])):
            raise C0B5LineageError("parent path contains an unsafe component")
        try:
            target.relative_to(root)
        except ValueError as exc:
            raise C0B5LineageError("parent path escapes benchmark root") from exc
        if target == root:
            raise C0B5LineageError("parent file path names the benchmark root")
        ...
        for index, component in enumerate(target.parent.parts[1:], start=1):
            ...
            if not stat.S_ISDIR(child_stat.st_mode):
                os.close(child)
                raise C0B5LineageError("parent path contains a non-directory")
            if index >= root_depth and (
                    child_stat.st_uid != os.getuid()
                    or stat.S_IMODE(child_stat.st_mode) & 0o077):
                os.close(child)
                raise C0B5LineageError("benchmark parent directory is not owner-only")
        ...
        if (not stat.S_ISREG(self._initial.st_mode)
            ...
            raise C0B5LineageError("parent must be an owner-only regular file")
        ...
        if self.conn.execute("PRAGMA integrity_check").fetchone() != ("ok",):
            raise C0B5LineageError("parent SQLite integrity check failed")
        if self.conn.execute("PRAGMA foreign_key_check").fetchall():
            raise C0B5LineageError("parent SQLite foreign-key check failed")
        return self

    def _assert_stable(self) -> None:
        if self.fd < 0 or self._initial is None:
            raise C0B5LineageError("parent descriptor is not open")
        for parent_fd, name, child_fd in self._bindings:
            ...
            if (named.st_dev, named.st_ino) != (pinned.st_dev, pinned.st_ino):
                raise C0B5LineageError("parent directory changed during verification")
        named = os.stat(self.path, follow_symlinks=False)
        current = os.fstat(self.fd)
        stable = ("st_dev", "st_ino", "st_size", "st_mtime_ns", "st_ctime_ns")
        if any(getattr(named, key) != getattr(current, key) for key in stable):
            raise C0B5LineageError("parent path changed during verification")
        if any(getattr(self._initial, key) != getattr(current, key) for key in stable):
            raise C0B5LineageError("parent file changed during verification")
        if _sha256_fd(self.fd) != self.sha256:
            raise C0B5LineageError("parent bytes changed during verification")
...
def _header(conn: sqlite3.Connection) -> tuple[dict[str, Any], str]:
    ...
    if len(rows) != 1 or type(rows[0][0]) is not str or type(rows[0][1]) is not str:
        raise C0B5LineageError("parent run-header census is invalid")
    try:
        value = json.loads(rows[0][0])
    except (TypeError, json.JSONDecodeError) as exc:
        raise C0B5LineageError("parent run header is invalid JSON") from exc
    digest = hashlib.sha256(_canonical(value)).hexdigest()
    if rows[0][0].encode("utf-8") != _canonical(value) or rows[0][1] != digest:
        raise C0B5LineageError("parent run header is noncanonical or changed")
    return value, digest
...
def _verify_c0b3_header(conn: sqlite3.Connection) -> None:
    ...
    if digest != expected["run_header_sha256"] or any(
        ...
        raise C0B5LineageError("C0B-3 run-header literal changed")
...
def _default_c0b3_verifier(
    ...
    if result.get("ok") is not True or result.get("errors") != []:
        raise C0B5LineageError("full C0B-3 parent verification failed")
    return result
...
def verify_parents_readonly(
    ...
    from .c0b5_replay import replay_c0b3_d50_connection, replay_c0b4_connection
    ...
    if any(path == root or root not in path.parents for path in (
        ...
        raise C0B5LineageError("parent paths do not share one benchmark root")
    ...
    with _PinnedSQLite(paths.c0b3_checkpoint, root) as c3_db, \
        ...
        if (c3_db.sha256 != FROZEN_EXECUTION_PARENT["checkpoint_sha256"]
            ...
            raise C0B5LineageError("parent checkpoint or snapshot digest changed")
        ...
        if result is not None and isinstance(result, Mapping) \
            ...
            raise C0B5LineageError("C0B-3 verifier rejected the execution parent")
        ...
        if c3_facts != c3_snapshot_facts:
            raise C0B5LineageError("C0B-3 D50 checkpoint and snapshot replay differ")
        ...
        if checkpoint_facts.without_receipt() != snapshot_facts.without_receipt():
            raise C0B5LineageError("C0B-4 checkpoint and snapshot replay differ")
        if (checkpoint_facts.backup_snapshot_sha256 != c4_snapshot.sha256
            ...
            raise C0B5LineageError("C0B-4 receipt does not bind its snapshot")
    ...
    for expected in verification.files:
        with _PinnedSQLite(expected.path, root) as pinned:
            ...
            if observed != expected:
                raise C0B5LineageError("immutable parent evidence changed")

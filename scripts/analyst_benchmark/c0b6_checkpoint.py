"""Strict SQLite storage boundary for the C0B-6 confirmation.

The checkpoint stores raw model responses only in an owner-only run directory. All
policy-sensitive public values use the closed C0B-6 artifact family and are validated on
write and replayed again by :mod:`c0b6_replay`.
...
from .c0b6_policy import BENCHMARK_PROTOCOL_ID, POLICY_ID, POLICY_SHA256
from .c0b6_schema import validate_artifact

PROTOCOL_ID = BENCHMARK_PROTOCOL_ID
SCHEMA_VERSION = 1
HEADER_VERSION = "c0b6-run-header-v1"
RUN_ID_RE = re.compile(r"c0b6-[0-9]{8}-[0-9]{6}-[0-9a-f]{24}")
...
class C0B6CheckpointError(RuntimeError):
    """The checkpoint is absent, malformed, stale, or internally inconsistent."""


class C0B6BudgetError(C0B6CheckpointError):
...
def _sha(value: Any, label: str) -> str:
    if type(value) is not str or not SHA_RE.fullmatch(value):
        raise C0B6CheckpointError(f"{label} must be lowercase SHA-256")
    return value
...
def _decode(raw: str, label: str) -> dict[str, Any]:
    try:
        ...
        raise C0B6CheckpointError(f"{label} is not canonical JSON") from exc
    if type(value) is not dict or canonical_json(value) != raw:
        raise C0B6CheckpointError(f"{label} is not a canonical object")
    return value


def validate_header(value: Mapping[str, Any]) -> dict[str, Any]:
    """Validate the strict C0B-6 header and storage constants."""
    try:
        normalized = validate_artifact(value)
    except (TypeError, ValueError) as exc:
        raise C0B6CheckpointError("run header violates the C0B-6 schema") from exc
    if (normalized.get("version") != HEADER_VERSION
        ...
        raise C0B6CheckpointError("run header identity or limits changed")
    return normalized
...
def _assert_named(path: Path, fd: int) -> None:
    ...
    if (named.st_dev, named.st_ino) != (pinned.st_dev, pinned.st_ino):
        raise C0B6CheckpointError("pinned file path changed during verification")
...
def validate_run_lineage(conn: sqlite3.Connection,
    ...
    if (len(invocations) > INVOCATION_CAPS["total"]
        ...
        raise C0B6CheckpointError("invocation ledger changed")
    ...
    if len(attempt_rows) > CUMULATIVE_CAP:
        raise C0B6CheckpointError("cumulative call allowance changed")
    for call_class, allowance in LEDGER_LIMITS.items():
        if sum(row[1] == call_class for row in attempt_rows) > allowance:
            raise C0B6CheckpointError("class call allowance changed")
    for attempt_id, call_class, ordinal, request_sha, outcome, payload in attempt_rows:
        ...
        if (call_class not in LEDGER_LIMITS or ordinal not in invocations
            ...
            raise C0B6CheckpointError("attempt ledger contains an invalid row")
        ...
        if not history or history[0] != ("DISPATCHING", None) \
            ...
            raise C0B6CheckpointError("attempt history differs from current row")
    if sum(row[4] == "DISPATCHING" for row in attempt_rows) > 1:
        raise C0B6CheckpointError("more than one request is in flight")
    for kind, owner, digest, raw in conn.execute(
        ...
        try:
            ...
            raise C0B6CheckpointError("stored artifact violates C0B-6 schema") from exc
        if (normalized != value or sha256_json(value) != digest
            ...
            raise C0B6CheckpointError("stored artifact digest changed")
    ...
    for anchor_raw, anchor_sha, receipt_raw, receipt_sha in conn.execute(
        ...
        if (anchor.get("anchor_sha256") != anchor_sha
            ...
            raise C0B6CheckpointError("backup receipt chain changed")
    if state_rows[0][0] in TERMINAL_STATES and any(
            row[4] == "DISPATCHING" for row in attempt_rows):
        raise C0B6CheckpointError("terminal state owns an in-flight attempt")
...
class C0B6Checkpoint:
    """One validated writable C0B-6 checkpoint handle."""
    ...
    @classmethod
    def open(cls, path: Path, *, writable: bool = False) -> "C0B6Checkpoint":
        path = Path(path)
        if (path.name != "checkpoint.sqlite3" or path.parent.parent.name != "runs"
                or not RUN_ID_RE.fullmatch(path.parent.name)):
            raise PermissionError("checkpoint is outside a C0B-6 run directory")
        ...
        try:
            ...
            if header["run_id"] != path.parent.name:
                raise C0B6CheckpointError("storage/run identity mismatch")
    ...
    def __enter__(self) -> "C0B6Checkpoint":
        return self
    ...
    def header(self) -> dict[str, Any]:
        ...
        if sha256_json(value) != row[1]:
            raise C0B6CheckpointError("run header digest changed")
        return value
        ...
        for path, digest in zip(self.parent_paths(), expected, strict=True):
            ...
            try:
                if _hash_fd(fd) != digest:
                    raise C0B6CheckpointError("immutable parent evidence changed")
                _assert_named(path, fd)
            finally:
                os.close(fd)
    ...
    def begin_invocation(self) -> int:
        ...
        if self.state() != "RUNNING":
            raise C0B6CheckpointError("invocation claim requires RUNNING")
        current = self.conn.execute("SELECT count(*) FROM invocations").fetchone()[0]
        if current >= INVOCATION_CAPS["total"]:
            raise C0B6BudgetError("invocation allowance exhausted")
        ordinal = current + 1
        with self.conn:
    ...
    def store_artifact(self, kind: str, owner_id: str,
        ...
        try:
            ...
            raise C0B6CheckpointError("artifact violates C0B-6 schema") from exc
        ...
        with self.conn:
            ...
            if prior and prior != (digest, raw):
                raise C0B6CheckpointError("artifact owner is immutable")
            if not prior:
    ...
    def read_artifact(self, kind: str, owner_id: str) -> dict[str, Any] | None:
        ...
        if sha256_json(value) != row[1]:
            raise C0B6CheckpointError("artifact digest changed")
        return value
    ...
    def set_nonce_key(self, value: bytes) -> None:
        ...
        if self.state() != "PREPARED":
            raise C0B6CheckpointError("nonce key may only be frozen before execution")
    ...
    def nonce_key(self) -> bytes:
        ...
        if not row or hashlib.sha256(row[0]).hexdigest() != row[1] or len(row[0]) != 32:
            raise C0B6CheckpointError("protected nonce key changed")
        return bytes(row[0])
    ...
    def precharge(self, *, attempt_id: str, owner_id: str, call_class: str,
        ...
        self._assert_parents_unchanged()
        if self.state() != "RUNNING":
            raise C0B6CheckpointError("precharge requires RUNNING")
        _sha(attempt_id, "attempt_id")
        ...
        if self.conn.execute(
                "SELECT 1 FROM attempts WHERE state='DISPATCHING'").fetchone():
            raise C0B6CheckpointError("another request is already in flight")
        if not self.conn.execute(
                "SELECT 1 FROM invocations WHERE ordinal=?",
                (invocation_ordinal,)).fetchone():
            raise C0B6CheckpointError("attempt names an unknown invocation")
        ...
        if class_count >= LEDGER_LIMITS[call_class] or total >= CUMULATIVE_CAP:
            raise C0B6BudgetError("call allowance exhausted")
        now = time.time()
        with self.conn:
            self.conn.execute(
    ...
    def record_attempt(self, attempt_id: str, state: str,
        ...
        with self.conn:
            ...
            if changed != 1:
                raise C0B6CheckpointError("attempt is absent or already final")
            self.conn.execute(
    ...
    def record_cancelled_attempt(
        ...
        if (type(first_byte_seen) is not bool or type(cancel_elapsed_ms) is not int
            ...
            raise C0B6CheckpointError("cancellation timing values are invalid")
        self._assert_parents_unchanged()
        now = time.time()
    ...
    def store_runtime_event(self, value: Mapping[str, Any]) -> str:
        normalized = validate_artifact(value)
        if normalized.get("version") != "c0b6-runtime-event-v1":
            raise C0B6CheckpointError("runtime event has the wrong artifact family")
        self._assert_parents_unchanged()
        raw = _json(normalized)
        with self.conn:
    ...
    def store_backup_receipt(self, anchor: Mapping[str, Any],
        ...
        if self.state() not in TERMINAL_STATES:
            raise C0B6CheckpointError("backup receipt requires a terminal state")
        normalized_anchor = validate_artifact(anchor)
        normalized_receipt = validate_artifact(receipt)
        if (normalized_anchor.get("version") != "c0b6-backup-anchor-v1"
                or normalized_receipt.get("version") != "c0b6-backup-receipt-v1"
                or normalized_receipt["anchor_sha256"] !=
                   normalized_anchor["anchor_sha256"]):
            raise C0B6CheckpointError("backup receipt chain is invalid")
    ...
    def parent_paths(self) -> tuple[Path, Path, Path, Path]:
        ...
        if not row:
            raise C0B6CheckpointError("parent paths are absent")
    ...
    def finalize(self, terminal: str, terminal_artifact: Mapping[str, Any], *,
        ...
        if terminal not in TERMINAL_STATES or self.state() not in ACTIVE_STATES:
            raise C0B6CheckpointError("invalid terminal transition")
        quality = terminal in {"CONFIRMED", "INCONCLUSIVE"}
        if quality != (completion is not None):
            raise C0B6CheckpointError("quality completion ownership differs")
        ...
        if (artifact.get("terminal") != terminal or any(
                artifact.get(key) != header[key] for key in (
                    "policy_id", "policy_sha256", "protocol_sha256"))):
            raise C0B6CheckpointError("terminal artifact differs from run lineage")
        ...
        if normalized is not None and (
            ...
            raise C0B6CheckpointError("completion does not own terminal result")
        self._assert_parents_unchanged()
        if self.conn.execute(
                "SELECT 1 FROM attempts WHERE state='DISPATCHING'").fetchone():
            raise C0B6CheckpointError("terminal transition owns an in-flight attempt")

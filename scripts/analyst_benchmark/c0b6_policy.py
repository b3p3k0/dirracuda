"""Frozen identity and review-budget policy for C0B-6.

This module is pure except for descriptor-safe reads used to derive the protocol
digest.  Historical C0B-2/C0B-3/C0B-4/C0B-5 identities remain distinct and cannot be
defaulted into the C0B-6 family.
...
POLICY_ID = "c0b6-assistive-bounded-fp-v1"
POLICY_SHA256 = "bf22aed2c077dec6e27ecd4121ca14c2e546c1a036855db7e3a58d6b1f2f55d3"
...
ACCEPTANCE_FAILURE_REASONS = (
    ...
    "f72_seed20260811_redundant_rows_above_1",
    "f72_seed20260811_affected_chunks_above_1",
    "f72_seed20260811_affected_documents_above_1",
...
_FAMILY_IDENTITIES = {
    ...
    "c0b5": (
        "c0b5-assistive-fp-confirmation-v1",
        "c0b5-assistive-bounded-fp-v1",
...
PROTOCOL_COMPONENT_PATHS = (
    ...
    ("c0b6_protocol_sha256", "docs/dev/ollama_integration/BENCHMARK_PROTOCOL_C0B6.md"),
    ...
    ("c0b5_outcome_sha256", "docs/dev/ollama_integration/PUBLIC_CDF_OUTCOME_C0B5.md"),
...
def current_policy(protocol_digest: str) -> ResolvedPolicy:
    return ResolvedPolicy(
        PolicyFamily.C0B6, BENCHMARK_PROTOCOL_ID, POLICY_ID, POLICY_SHA256,
...
def resolve_payload_policy(
    ...
    if present != _PAYLOAD_FIELDS:
        raise PolicyIdentityError("payload has a partial C0B-6 identity")
    ...
    if any(type(payload[key]) is not str or payload[key] != value
        ...
        raise PolicyIdentityError("payload C0B-6 identity is unknown or mismatched")
...
def resolve_header_policy(
    ...
    if present != _HEADER_FIELDS:
        raise PolicyIdentityError("header has a partial C0B-6 identity")
    ...
    if any(type(header[key]) is not str or header[key] != value
        ...
        raise PolicyIdentityError("header C0B-6 identity is unknown or mismatched")
    if expected_protocol_sha256 is not None \
            and digest != _sha256(expected_protocol_sha256, "expected protocol digest"):
...
def require_current_payload(
    ...
    if value.family != PolicyFamily.C0B6:
        raise PolicyIdentityError("C0B-6 operation requires a C0B-6 payload")
    return value
...
def require_current_header(
    ...
    if value.family != PolicyFamily.C0B6:
        raise PolicyIdentityError("C0B-6 operation requires a C0B-6 header")
    return value


def protocol_identity_payload(repo_root: Path) -> dict[str, Any]:
    """Build the exact ten-component protocol preimage."""
...
if hashlib.sha256(canonical_json_bytes(policy_payload())).hexdigest() != POLICY_SHA256:
    raise RuntimeError("frozen C0B-6 policy differs from its declared SHA-256")

"""Non-circular terminal backup receipts for C0B-6 checkpoints.
...
ANCHOR_VERSION = "c0b6-backup-anchor-v1"
RECEIPT_VERSION = "c0b6-backup-receipt-v1"
...
class C0B6BackupError(C0B6CheckpointError):
    """A C0B-6 terminal snapshot or receipt is invalid."""
...
def source_binding_from_header(header: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if frozenset(source) != SOURCE_KEYS:
        raise C0B6BackupError("C0B-6 source binding is incomplete")
    return source
...
def validate_anchor(value: Mapping[str, Any],
    ...
    try:
        ...
        raise C0B6BackupError("backup anchor violates its strict schema") from exc
    if (anchor.get("version") != ANCHOR_VERSION
        ...
        raise C0B6BackupError("backup anchor identity changed")
    return anchor
...
def validate_receipt(value: Mapping[str, Any],
    ...
    try:
        ...
        raise C0B6BackupError("backup receipt violates its strict schema") from exc
    ...
    if (receipt.get("version") != RECEIPT_VERSION
        ...
        raise C0B6BackupError("backup receipt identity changed")
    return receipt
...
def _terminal_rows(conn: sqlite3.Connection, state: str) -> tuple[str, str | None]:
    ...
    if quality:
        ...
        if (canonical_json(completion) != completions[0][1]
            ...
            raise C0B6BackupError("completion ownership changed")
        return str(rows[0][0]), str(completions[0][0])
    if completions:
        raise C0B6BackupError("failure terminal owns a quality completion")
...
def _verify_terminal(conn: sqlite3.Connection, state: str,
    ...
    if state not in TERMINAL_STATES:
        raise C0B6BackupError("backup source is not terminal")
    if conn.execute(
            "SELECT 1 FROM attempts WHERE state='DISPATCHING'").fetchone():
        raise C0B6BackupError("terminal owns an in-flight attempt")
...
def build_anchor(point: C0B6Checkpoint, *, terminal_artifact_sha256: str,
    ...
    if (terminal_artifact_sha256 != expected_terminal
        ...
        raise C0B6BackupError("anchor owner differs from terminal lineage")
...
def verify_snapshot(
    ...
    try:
        ...
        if (not stat.S_ISREG(st.st_mode) or st.st_uid != os.getuid()
            ...
            raise C0B6BackupError("backup snapshot mode, size, or digest changed")
        ...
        try:
            ...
            if conn.execute("PRAGMA integrity_check").fetchall() != [("ok",)] \
                ...
                raise C0B6BackupError("backup snapshot failed SQLite checks")
            ...
            if (raw != canonical_json(header) or digest != anchor["header_sha256"]):
                raise C0B6BackupError("backup snapshot header changed")
            ...
            if (terminal != anchor["terminal_artifact_sha256"]
                ...
                raise C0B6BackupError("backup terminal owner changed")
        finally:
            conn.close()
        named = os.stat(name, dir_fd=backup_fd, follow_symlinks=False)
        if (named.st_dev, named.st_ino) != (st.st_dev, st.st_ino):
            raise C0B6BackupError("backup snapshot left its pinned inode")
...
def ensure_backup_receipt(
    ...
    if point.conn.in_transaction or point.state() not in TERMINAL_STATES:
        raise C0B6BackupError("backup requires committed terminal evidence")
    header = point.header()
    if semantic_verifier is None:
        from .c0b6_lineage import ParentPaths, verify_parents_readonly
        from .c0b6_replay import replay_c0b6_connection
        parents = verify_parents_readonly(ParentPaths(*point.parent_paths()))
        semantic_verifier = lambda conn: replay_c0b6_connection(
            conn, parent_facts=parents, require_receipt=False)
    # Semantic replay deliberately enables SQLite's connection-local query_only
    # guard. Never lend it the publication handle: C0B-5 proved that doing so can
    # make the later receipt INSERT deterministically read-only.
    with C0B6Checkpoint.open(point.path) as replay_point:
        semantic_verifier(replay_point.conn)
    anchor = build_anchor(
    ...
    if row:
        ...
        if (canonical_json(stored_anchor) != row[0]
            ...
            raise C0B6BackupError("stored backup row changed")
        verify_snapshot(
...
def verify_backup_readonly(
    ...
    try:
        with C0B6Checkpoint.open(path) as point:
            ...
            if len(rows) != 1:
                raise C0B6BackupError("terminal must have one receipt")
            ...
            if (canonical_json(anchor) != rows[0][0]
                ...
                raise C0B6BackupError("source receipt row changed")

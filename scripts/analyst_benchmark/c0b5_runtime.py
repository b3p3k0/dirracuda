"""Guarded creation, execution, and verification for C0B-5.
...
from .c0b5_backup import ensure_backup_receipt, verify_backup_readonly
...
from .c0b5_replay import (
    replay_c0b5_connection,
    verify_c0b5_terminal_readonly,
)
...
class C0B5RuntimeError(RuntimeError):
    """A runtime gate failed without exposing source or response content."""


class C0B5FilesystemError(C0B5RuntimeError):
...
def _task_tree_sha256(repo_root: Path) -> str:
    rows = {relative: _file_sha256(repo_root / relative, root=repo_root)
            for relative in sorted(FROZEN_C0B5_PUBLIC_PATHS)}
    return stable_hash(rows)


def _require_clean_source(seal: WorktreeSeal) -> None:
    if any(row.path in FROZEN_C0B5_PUBLIC_PATHS for row in seal.entries):
        raise C0B5RuntimeError("commit the frozen C0B-5 implementation before create")


def _generation_options_sha256() -> str:
    rows = []
    for lane_id, seed in zip(LANE_ORDER, (20260804, 20260811, 1), strict=True):
        rows.append({
            "lane_id": lane_id,
    ...
    detector_hash = stable_hash({
        ...
        **{name: _file_sha256(
           ...
           for name in ("c0b2_public_scoring.py", "c0b2_stage_c.py",
                        ...
                        "c0b4_answer.py", "c0b5_scoring.py")},
    })
    return {
        ...
        "schema_sha256": stable_hash({
            ...
            "c0b5": _file_sha256(
                repo_root / "scripts/analyst_benchmark/c0b5_schema.py",
                root=repo_root),
        }),
...
def _find_snapshot(run_dir: Path, expected_sha256: str) -> Path:
    ...
    if len(matches) != 1:
        raise C0B5RuntimeError("frozen parent snapshot is absent or ambiguous")
    return matches[0]
...
def _header(*, run_id: str, source: Mapping[str, Any],
    ...
    if filesystem.selected_mode != JOURNAL_MODE:
        raise C0B5RuntimeError("benchmark filesystem did not pass DELETE+FULL")
    value = {
...
def _new_run_id() -> str:
    return f"{time.strftime('c0b5-%Y%m%d-%H%M%S', time.gmtime())}-{secrets.token_hex(12)}"
...
def _assert_child_allowance(root: Path) -> None:
    ...
    if any(RUN_ID_RE.fullmatch(name) for name in names):
        raise C0B5RuntimeError("the one-child C0B-5 allowance is already owned")
    if any(name.startswith(".c0b5-initializing-") for name in names):
        raise C0B5RuntimeError("staged C0B-5 evidence requires review")
...
def _corpus(header: Mapping[str, Any] | None = None) -> Any:
    ...
    if header is not None and manifest.sha256 != header["master_manifest_sha256"]:
        raise C0B5RuntimeError("public corpus manifest changed")
...
def create_confirmation_run(
    ...
    """Create the complete C0B-5 child without contacting Ollama."""
    ...
    if not RUN_ID_RE.fullmatch(identity):
        raise ValueError("invalid C0B-5 run id")
    ...
    if type(key) is not bytes or len(key) != 32:
        raise ValueError("C0B-5 nonce key must contain exactly 32 bytes")
    ...
    def initialize(point: C0B5Checkpoint) -> None:
...
def confirmation_verify(
    ...
    try:
        if errors:
            raise C0B5RuntimeError("checkpoint structural verification failed")
        with C0B5Checkpoint.open(path) as point:
            ...
            if point.state() not in TERMINAL_STATES:
                ...
                if not checked.get("ok"):
                    raise C0B5RuntimeError("terminal backup verification failed")
                facts = verify_c0b5_terminal_readonly(
...
def revalidate_source_pins(header: Mapping[str, Any], *,
    ...
    if any(header.get(key) != item for key, item in current.items()):
        raise C0B5RuntimeError("immutable C0B-5 source identity drift")
    try:
        ...
        raise C0B5FilesystemError("filesystem capability changed") from exc
...
def _artifact(point: C0B5Checkpoint, kind: str,
              owner: str) -> dict[str, Any] | None:
    return point.read_artifact(kind, owner)


def _public_result(point: C0B5Checkpoint) -> dict[str, Any]:
    return {
...
class _Resolver:
    ...
    def __call__(self, request: WorkRequest | ControlRequest) -> RequestSpec:
        ...
        try:
            ...
            raise C0B5RuntimeError("request names an unregistered control") from None
...
def _attempt_no(point: C0B5Checkpoint, owner_id: str) -> int:
...
def _call_control(
        point: C0B5Checkpoint, resolver: _Resolver, transport: Callable[..., Any],
...
def _run_preflight(point: C0B5Checkpoint, resolver: _Resolver,
                   transport: Callable[..., Any], ordinal: int,
                   before_call: Callable[[], bool] | None = None,
                   ) -> tuple[str, str | None]:
    for kind, spec in _preflight_specs(point.header()):
        owner = stable_hash({"c0b5_preflight": kind, "invocation": ordinal})
...
def _validate_execution_history(point: C0B5Checkpoint, resolver: _Resolver, *,
    ...
    for ordinal in {row["invocation_ordinal"] for row in attempts}:
        for name, spec in _preflight_specs(point.header()):
            preflight[stable_hash({
                "c0b5_preflight": name, "invocation": ordinal})] = spec
    attempt_by_id = {}
    for row in attempts:
        owner = row["owner_id"]
        ...
        if owner in resolver.work_ids:
            ...
            raise C0B5RuntimeError("attempt owner is outside the frozen request tree")
        if (row["request_sha256"] != expected_request
                or row["attempt_id"] != expected_attempt
                or row["attempt_id"] in attempt_by_id):
            raise C0B5RuntimeError("attempt identity differs from frozen request")
    ...
    for event in point.list_runtime_events():
        ...
        if attempt is None or attempt["owner_id"] not in resolver.work_ids:
            raise C0B5RuntimeError("runtime event names no scored attempt")
        ...
        if (key in existing or event["lane_id"] != lane
            ...
            raise C0B5RuntimeError("runtime event differs from its attempt")
        existing.add(key)
    if require_complete_events:
        for attempt in attempts:
            ...
            if not expected.issubset(existing):
                raise C0B5RuntimeError("runtime event history is incomplete")


def _assessment_rows(point: C0B5Checkpoint, work: Mapping[str, Any]
...
def _work_evidence(point: C0B5Checkpoint, work: Mapping[str, Any], *,
    ...
    if not prompts:
        raise C0B5RuntimeError("answered work lacks prompt-evaluation evidence")
    ...
    if outcome == "NORMALIZED_DUPLICATE":
        dedup = {
            "version": "c0b5-dedup-evidence-v1", **_identity(point.header()),
        ...
        if existing is None and not persist_dedup:
            raise C0B5RuntimeError("normalized attempt lacks dedup artifact")
        ...
        if existing is not None and canonical_json(existing) != canonical_json(dedup):
            raise C0B5RuntimeError("stored dedup evidence does not rederive")
    chunk = {
    ...
    parsed = C0B5ChunkRow.model_validate(chunk, strict=True).model_dump(mode="json")
...
def _first_answered_lane_work(
        point: C0B5Checkpoint, resolver: _Resolver,
...
def _next_work_disposition(point: C0B5Checkpoint, work: Mapping[str, Any],
...
def _ensure_context(
    ...
    def validate_response(response: FakeResponse) -> None:
        ...
        built["evidence"] = ContextEvidence.model_validate({
            "version": "c0b5-context-evidence-v1", **_identity(point.header()),
            "control_id": frozen["control_id"], "lane_id": "F72_20260804",
    ...
    if durable:
        if len(durable) != 1:
            raise C0B5RuntimeError("context response census is not exact")
    ...
    if existing is not None and canonical_json(existing) != canonical_json(
            built["evidence"]):
        raise C0B5RuntimeError("context evidence does not rederive")
...
def _run_lane_work(
    ...
    if lane_id == "F72_20260804":
        ...
        if _artifact(point, "context_evidence",
                ...
                and trigger is None:
            raise C0B5RuntimeError("context evidence lacks an answered trigger")
    ...
    for planned in lane["work"]:
        ...
        def precharge(request: WorkRequest) -> None:
            point.precharge(
                ...
                request_sha256=request.request_hash)
            from .c0b5_executor import runtime_event
        ...
        if lane_id == "F72_20260804" and _artifact(
...
def _validate_health_response(response: FakeResponse) -> None:
    ...
    if (type(metadata.get("done_reason")) is not str
            ...
            or any(type(metadata.get(key)) is not bool for key in (
                "tools_empty", "images_empty", "unknown_message_fields_empty"))):
        raise C0B5RuntimeError("health response metadata is incomplete")


def _derive_cancel_health(point: C0B5Checkpoint,
    ...
    if len(cancelled) != 1:
        raise C0B5RuntimeError("cancellation evidence census is not exact")
    ...
    answered = [(row, assess_answer(
        ...
        if type((row["payload"] or {}).get("response")) is str]
    if not answered:
        raise C0B5RuntimeError("health evidence is absent")
    ...
    value = {
        ...
        **_identity(point.header()), "lane_id": "F72_20260804",
...
def _run_cancel_health(
    ...
    if existing is not None:
        if canonical_json(existing) != canonical_json(
            ...
            raise C0B5RuntimeError("cancellation evidence does not rederive")
...
def _verify_existing_control_evidence(
    ...
    if _artifact(point, "context_evidence", context_id) is not None:
        lane = point.read_artifact("lane_plan", "F72_20260804")
        trigger = _first_answered_lane_work(point, resolver, lane)
        if trigger is None:
            raise C0B5RuntimeError("context evidence lacks an answered trigger")
        state, _owner = _ensure_context(
            ...
            lambda *_args, **_kwargs: (_ for _ in ()).throw(
                C0B5RuntimeError("control replay attempted HTTP")),
            ordinal=0, controls=controls, trigger_work=trigger[0],
            trigger_attempt_id=trigger[1])
        if state != "RAW_VALID":
            raise C0B5RuntimeError("context evidence is not reusable")
...
def _lane_evidence(point: C0B5Checkpoint, lane: Mapping[str, Any],
    ...
    for planned in lane["work"]:
        ...
        if _next_work_disposition(point, work, ids)[0] != "complete":
            raise C0B5RuntimeError("lane aggregation preceded complete evidence")
        evidence[work["work_id"]] = _work_evidence(
            point, work, persist_dedup=persist_dedup)
    return evidence


def _activate_lane(point: C0B5Checkpoint, master: Mapping[str, Any],
    ...
    value = validate_artifact({
        "version": "c0b5-plan-activation-v1", **_identity(point.header()),
...
def _cursor_transition(point: C0B5Checkpoint, *, lane: Mapping[str, Any],
                       next_lane: Mapping[str, Any], aggregate_sha256: str) -> str:
    existing = _artifact(point, "cursor_transition", lane["lane_id"])
    value: dict[str, Any] = {
        "version": "c0b5-cursor-transition-v1", **_identity(point.header()),
...
def _finish_quality(
    ...
    result = validate_artifact({
        "version": "c0b5-result-v1", **_identity(point.header()),
        "terminal": terminal, "reason": reason,
        "master_plan_sha256": sha256_json(master),
        "lane_aggregate_sha256s": {
            "f72_seed20260804_sha256": first_hash,
            "f72_seed20260811_sha256": second_hash,
    ...
    completion = validate_artifact({
        "version": "c0b5-completion-v1", **_identity(point.header()),
...
def _finish_failure(
        point: C0B5Checkpoint, lock: GlobalExecutionLock, *, terminal: str,
    ...
    evidence: dict[str, Any] = {
        "version": "c0b5-failure-evidence-v1", **_identity(point.header()),
        "terminal": terminal, "reason": reasons[terminal], "lane_id": lane_id,
    ...
    failure = validate_artifact({
        "version": "c0b5-failure-v1", **_identity(point.header()),
...
def _ensure_terminal_backup(point: C0B5Checkpoint,
    ...
    if terminal is None:
        raise C0B5RuntimeError("terminal state lacks its owner artifact")
...
def _recheck_before_terminal(point: C0B5Checkpoint, *, repo_root: Path) -> None:
    revalidate_source_pins(point.header(), repo_root=repo_root)
    verify_parents_readonly(ParentPaths(*point.parent_paths()))


def _pause(point: C0B5Checkpoint, state: str) -> dict[str, Any]:
    point.transition(state)
    return _public_result(point)


def _blocked_parent_drift(point: C0B5Checkpoint) -> dict[str, Any]:
...
def _derive_lane(
    ...
    if lane["lane_id"] != "F72_20260804":
        return build_lane_aggregate(lane, evidence, corpus=corpus), None, None
    trigger = _first_answered_lane_work(point, resolver, lane)
    if trigger is None:
        raise C0B5RuntimeError("first lane lacks context evidence")
    state, owner = _ensure_context(
        point, resolver, transport, ordinal=ordinal, controls=controls,
    ...
    if not allow_control_contact and _artifact(
        ...
        raise C0B5RuntimeError("passing first lane lacks cancellation evidence")
...
def run_confirmation(
    ...
    """Run or explicitly resume the exact serial C0B-5 schedule."""
    ...
    with GlobalExecutionLock(root) as lock:
        with C0B5Checkpoint.open(_checkpoint_path(run_id, root), writable=True) as point:
            ...
            if point.state() != "PREPARED" and not resume:
                raise C0B5RuntimeError("non-prepared confirmation requires --resume")
            try:
                revalidate_source_pins(point.header(), repo_root=repo_root)
                paths = ParentPaths(*point.parent_paths())
            except C0B5FilesystemError:
            ...
            except C0B5BudgetError:
                return _finish_failure(
                    point, lock, terminal="BLOCKED_BUDGET", repo_root=repo_root)
            ...
            for index, lane_id in enumerate(LANE_ORDER):
                ...
                try:
                    ...
                    if aggregate is not None and canonical_json(
                        ...
                        raise C0B5RuntimeError("stored aggregate does not rederive")
                    aggregate = derived
                ...
                if lane_id.startswith("F72") and not aggregate["passed"]:
                    reason = ("seed20260804_control_gate_failed"
                              if lane_id == "F72_20260804" and
                              aggregate["failure_reasons"] == [
                                  "cancellation_health_failure"] else
                              "seed20260804_no_qualifier" if
                              lane_id == "F72_20260804" else
                              "seed20260811_no_qualifier")
                    return _finish_quality(
                        point, lock, master=master, terminal="INCONCLUSIVE",
                        reason=reason,
                        first_hash=lane_hashes["F72_20260804"],
                        second_hash=lane_hashes.get("F72_20260811"),
                        repo_root=repo_root)
...
def abandon_confirmation_run(
    ...
    with GlobalExecutionLock(root) as lock:
        with C0B5Checkpoint.open(_checkpoint_path(run_id, root), writable=True) as point:
            ...
            _recheck_before_terminal(point, repo_root=repo_root)
            return _finish_failure(
                point, lock, terminal="ABANDONED", repo_root=repo_root)

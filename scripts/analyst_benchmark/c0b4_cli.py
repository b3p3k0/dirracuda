"""Fail-closed command surface for the C0B-4 public confirmation.

Private execution remains outside this namespace.  Imports that can resolve user paths
or construct transports occur only after the command's explicit confirmation gate.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m scripts.analyst_benchmark c0b4",
        description="C0B-4 grounded-duplicate public confirmation.",
    )
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("create", help="create the frozen C0B-4 child run")
    for name in ("status", "verify"):
        command = commands.add_parser(name, help=f"{name} a C0B-4 run")
        _add_run_id(command)
    for name in ("run", "resume"):
        command = commands.add_parser(name, help=f"{name} the C0B-4 run")
        _add_run_id(command)
        command.add_argument("--confirm-live", action="store_true")
    abandon = commands.add_parser("abandon", help="abandon a C0B-4 run")
    _add_run_id(abandon)
    abandon.add_argument("--confirm-abandon", action="store_true")
    leak = commands.add_parser("leak-scan", help="scan the exact C0B-4 task delta")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command in {"run", "resume"} and not args.confirm_live:
        print("--confirm-live is required before any C0B-4 live operation.",
              file=sys.stderr)
        return EXIT_USAGE
    ...
    try:
        ...
        runtime = importlib.import_module("scripts.analyst_benchmark.c0b4_runtime")
        ...
        print("C0B-4 BLOCKED: operation_failed", file=sys.stderr)
        return EXIT_BLOCKED

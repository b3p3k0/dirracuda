...
from .c0b2_leakscan import (
    FROZEN_C0B2_PUBLIC_PATHS, FROZEN_C0B3_PUBLIC_PATHS,
    FROZEN_C0B4_PUBLIC_PATHS, FROZEN_C0B5_PUBLIC_PATHS,
    FROZEN_C0B6_PUBLIC_PATHS, FROZEN_C0B7_PUBLIC_PATHS,
...
C0B6_PROTOCOL_ID = "c0b6-repaired-confirmation-v1"
C0B7_PROTOCOL_ID = "c0b7-offline-recovery-v1"
...
C0B6_ALLOWLIST_EXACT: Set[str] = set(FROZEN_C0B6_PUBLIC_PATHS)
C0B7_ALLOWLIST_EXACT: Set[str] = set(FROZEN_C0B7_PUBLIC_PATHS)
...
def allowed(path: str, protocol_id: str | None = None) -> bool:
    if protocol_id not in {
            ...
            C0B6_PROTOCOL_ID, C0B7_PROTOCOL_ID}:
        raise BaselineError("unknown leak-scan protocol identity")
    if protocol_id == C0B7_PROTOCOL_ID:
        exact = C0B7_ALLOWLIST_EXACT
    elif protocol_id == C0B6_PROTOCOL_ID:
        exact = C0B6_ALLOWLIST_EXACT
    elif protocol_id == C0B5_PROTOCOL_ID:
...
def run(*, baseline_path: Path, raw_artifacts: Sequence[Path],
    ...
    try:
        baseline = load_baseline(
            baseline_path,
            allow_direct_parent=protocol_id in {
                C0B4_PROTOCOL_ID, C0B5_PROTOCOL_ID, C0B6_PROTOCOL_ID,
                C0B7_PROTOCOL_ID})

"""Pure scoring for the C0B-5 assistive review-budget confirmation.
...
from .c0b5_plan import LANE_CONFIG, SELECTION, candidate_id
...
from .c0b5_schema import (
    AcceptanceAggregate, C0B5ChunkRow, C0B5PublicWork, C44ScoredAggregate,
...
_FP_COMPONENTS = (
    "C44_RERUN", "D50_CONFIRMATION", "F72_SEED20260804",
    "F72_SEED20260811",
)
_TEMPLATE_RULES = {
    "neg_clean_": {
...
class C0B5ScoringError(RuntimeError):
...
def _sha256(value: Any, label: str, *, nullable: bool = False) -> str | None:
    ...
    if (type(value) is not str or len(value) != 64
        ...
        raise C0B5ScoringError(f"{label} must be lowercase SHA-256")
    return value
...
def _identity(value: Mapping[str, Any]) -> dict[str, str]:
    if (value.get("policy_id"), value.get("policy_sha256")) != (
            POLICY_ID, POLICY_SHA256):
        raise C0B5ScoringError("artifact policy identity is not C0B-5")
    return {
...
def _model(model: Any, value: Mapping[str, Any], label: str) -> dict[str, Any]:
    try:
        ...
        raise C0B5ScoringError(f"{label} violates its strict C0B-5 schema") from exc
...
def _plan_digest(plan: Mapping[str, Any]) -> str:
    ...
    if stored != digest:
        raise C0B5ScoringError("lane plan self-digest is invalid")
    return digest
...
def _validate_plan(plan: Mapping[str, Any], corpus: PublicCorpus) -> dict[str, Any]:
    if type(plan) is not dict or plan.get("lane_id") not in LANE_CONFIG:
        raise C0B5ScoringError("lane plan is not an exact C0B-5 lane")
    lane_id = plan["lane_id"]
    expected_version = (
        "c0b5-acceptance-plan-v1" if lane_id == "C44_1"
    ...
    if set(plan) != expected_keys or plan["version"] != expected_version:
        raise C0B5ScoringError("lane plan has an invalid shape or version")
    _identity(plan)
    ...
    if corpus.master_manifest_sha256 != \
        ...
        raise C0B5ScoringError("lane corpus differs from parent manifest")
    if canonical_json(plan["candidate"]) != canonical_json(SELECTION):
        raise C0B5ScoringError("lane candidate differs from finalist")
    seed, plan_key, _domain, count = LANE_CONFIG[lane_id]
    if plan["seed"] != seed or type(plan["work"]) is not list:
        raise C0B5ScoringError("lane seed or work type is invalid")
    work = [_model(C0B5PublicWork, row, "lane work") for row in plan["work"]]
    ...
    if (len(work) != count or len({row["work_id"] for row in work}) != count
        ...
        raise C0B5ScoringError("lane work census/order differs from corpus split")
    _plan_digest(plan)
...
def _normalized_chunk(work: Mapping[str, Any], evidence: Mapping[str, Any],
    ...
    if type(evidence) is not dict or set(evidence) != {
        ...
        raise C0B5ScoringError("work evidence has an inexact shape")
    raw = evidence["chunk"]
    extras = {
    ...
    if type(raw) is not dict or set(raw) != set(ChunkRow.model_fields) | extras:
        raise C0B5ScoringError("C0B-5 chunk row has an invalid shape")
    hardened = _model(C0B5ChunkRow, raw, "chunk")
    old = {key: hardened[key] for key in ChunkRow.model_fields}
    if old["work_id"] != work["work_id"] or old["doc_id"] != work["doc_id"]:
        raise C0B5ScoringError("chunk evidence differs from planned work")
    ...
    if normalized:
        ...
        if (dedup["work_id"] != work["work_id"]
            ...
            raise C0B5ScoringError("dedup evidence differs from chunk facts")
    elif dedup is not None:
        raise C0B5ScoringError("non-normalized chunk carries dedup artifact")
    findings = evidence["retained_findings"]
    if type(findings) is not list or len(findings) > 16:
        raise C0B5ScoringError("retained finding list is malformed or unbounded")
    ...
    for finding in findings:
        if (type(finding) is not dict or set(finding) != {"category", "quote"}
            ...
            raise C0B5ScoringError("retained finding has an invalid shape")
...
def _documents(plan: Mapping[str, Any], evidence_by_work: Mapping[str, Any],
    ...
    if (type(evidence_by_work) is not dict
        ...
        raise C0B5ScoringError("evidence must exactly cover planned work")
    ...
    if list(grouped) != list(order):
        raise C0B5ScoringError("document evidence differs from manifest order")
...
def _build_lane_aggregate(
    ...
    if controls_passed is not None and type(controls_passed) is not bool:
        raise C0B5ScoringError("control pass flag must be Boolean or not-run")
    lane_id, assigned = parsed["lane_id"], parsed["lane_id"] == "F72_20260804"
    if assigned:
        ...
        raise C0B5ScoringError("unassigned lane carries control evidence")
    elif controls_passed is None:
        raise C0B5ScoringError("unassigned lane cannot have not-run controls")
    documents, canonical = _documents(parsed, evidence_by_work, corpus)
    if not canonical and lane_id != "C44_1":
        raise C0B5ScoringError("F72 retained evidence is noncanonical")
    ...
    if lane_id == "C44_1":
        ...
        if assigned and controls_passed is None:
            if cancellation_health_evidence_sha256 is not None:
                raise C0B5ScoringError("not-run cancellation carries evidence")
            if not preliminary:
                if precontrol_probe:
                    return None
                raise C0B5ScoringError(
                    "passing seed-20260804 lane requires cancellation evidence")
        elif assigned and cancellation_health_evidence_sha256 is None:
            raise C0B5ScoringError(
                "completed seed-20260804 controls require cancellation evidence")
    ...
    if lane_id == "C44_1":
        value = {
            "version": "c0b5-c44-scored-v1", **common,
    ...
    value = {
        "version": "c0b5-lane-aggregate-v1", **common,
...
def build_lane_aggregate(
    ...
    if value is None:
        raise C0B5ScoringError("final lane aggregate is unexpectedly absent")
    return value
...
def validate_lane_aggregate(
    ...
    if type(stored) is not dict or canonical_json(stored) != canonical_json(exact):
        raise C0B5ScoringError("stored lane aggregate is not exact")
    return exact
...
def template_family(document_id: str) -> str:
    ...
    if type(document_id) is not str:
        raise C0B5ScoringError("negative document ID must be exact text")
    ...
    raise C0B5ScoringError("negative document ID is outside frozen template rules")
...
def false_positive_rows(lane: Mapping[str, Any], *, component: str) -> list[dict[str, Any]]:
    ...
    if component not in _FP_COMPONENTS:
        raise C0B5ScoringError("false-positive component is unknown")
    documents = lane.get("retained_metrics", {}).get("documents")
    if type(documents) is not list:
        raise C0B5ScoringError("lane documents are absent")
    rows = [{
    ...
    if sum(row["negative_retained_findings"] for row in rows) != \
        ...
        raise C0B5ScoringError("public false-positive rows differ from lane totals")
    return rows
...
def _normalize_d50_component(value: Mapping[str, Any], *,
    ...
    if type(value) is not dict or set(value) != _COMPONENT_KEYS:
        raise C0B5ScoringError("D50 component has an invalid exact shape")
    ...
    if (value["component"] != "D50_CONFIRMATION"
        ...
        raise C0B5ScoringError("D50 component differs from frozen parent evidence")
...
def derive_parent_d50_component(
    ...
    if (sha256_json(final_d_decision) != parent["final_d_decision_sha256"]
        ...
        raise C0B5ScoringError("D50 inputs differ from frozen parent hashes")
    if type(negative_retained_findings) is not int or negative_retained_findings < 0:
        raise C0B5ScoringError("D50 negative retained count is invalid")
    try:
        ...
        raise C0B5ScoringError("D50 parent evidence does not rederive") from exc
...
def build_acceptance_aggregate(
        ...
        f72_seed20260804_lane: Mapping[str, Any], *, corpus: PublicCorpus,
    ...
    if any(type(value) is not bool for value in (
        ...
        raise C0B5ScoringError("acceptance attestations must be exact Booleans")
    if not provenance_passed or not safety_passed:
        raise C0B5ScoringError("provenance/safety failure requires its own terminal")
    c44_lane = _model(C44ScoredAggregate, c44_lane, "C44 aggregate")
    f72 = _model(LaneAggregate, f72_seed20260804_lane, "F72 aggregate")
    if f72["lane_id"] != "F72_20260804":
        raise C0B5ScoringError("acceptance requires seed-20260804 F72")
    if c44_lane["acceptance_plan_sha256"] != acceptance_plan_sha256:
        raise C0B5ScoringError("C44 aggregate differs from acceptance plan")
    if c44_lane["protocol_sha256"] != f72["protocol_sha256"]:
        raise C0B5ScoringError("acceptance lane protocol differs")
    c44 = _component_from_lane(c44_lane, component="C44_RERUN")
    f72_component = _component_from_lane(f72, component="F72_SEED20260804")
    ...
    if [row["document_ids"] for row in rows] != [
        ...
        raise C0B5ScoringError("acceptance components do not cover C/D/F exactly")
    if any(canonical_json(row["selection"]) != canonical_json(SELECTION)
           or row["candidate_id"] != candidate_id() for row in rows):
        raise C0B5ScoringError("acceptance component finalist identity differs")
    category = {name: {
    ...
    if any(row["support"] != 20 for row in category.values()):
        raise C0B5ScoringError("acceptance support differs from 20 per category")
    ...
    if totals["expected_chunks"] != 202:
        raise C0B5ScoringError("acceptance chunk census differs from 202")
    ...
    failures = {
        ...
        **{f"f72_seed20260804_{name}": f72_recovery[field] > 1
           for name, field in (
            ("redundant_rows_above_1", "redundant_rows"),
            ("affected_chunks_above_1", "affected_chunk_count"),
            ("affected_documents_above_1", "affected_document_count"))},
    ...
    value = {
        "version": "c0b5-acceptance-aggregate-v1",
        "policy_id": POLICY_ID, "policy_sha256": POLICY_SHA256,
        ...
        "component_hashes": {
            ...
            "f72_seed20260804_aggregate_sha256": sha256_json(f72),
        },
...
def build_public_summary(
        *, run_id: str, result: Mapping[str, Any], completion: Mapping[str, Any],
        f72_seed20260804_lane: Mapping[str, Any],
        d50_component: Mapping[str, Any],
        d50_false_positive_documents: Sequence[Mapping[str, Any]],
        corpus: PublicCorpus,
        f72_seed20260811_lane: Mapping[str, Any] | None = None,
    ...
    if type(run_id) is not str or not run_id:
        raise C0B5ScoringError("public summary run ID is invalid")
    result = _model(Result, result, "terminal result")
    completion = _model(Completion, completion, "completion")
    first = _model(LaneAggregate, f72_seed20260804_lane, "first F lane")
    d50 = _normalize_d50_component(d50_component, corpus=corpus)
    second = None if f72_seed20260811_lane is None else _model(
        LaneAggregate, f72_seed20260811_lane, "second F lane")
    ...
    if (first["lane_id"] != "F72_20260804"
            or second is not None and second["lane_id"] != "F72_20260811"
            or completion["artifact_sha256"] != result_sha
            or completion["outcome"] != result["terminal"]
            or result["lane_aggregate_sha256s"]["f72_seed20260804_sha256"] !=
               sha256_json(first)
        ...
        raise C0B5ScoringError("public summary source ownership differs")
    ...
    if any(type(row) is not dict or row.get("component") != "D50_CONFIRMATION"
           for row in d50_rows):
        raise C0B5ScoringError("D50 public rows have an invalid component")
    ...
    if (len(d50_rows) != d50["negative_false_positive_documents"]
        ...
        raise C0B5ScoringError("D50 public rows differ from replayed component")
    rows = false_positive_rows(first, component="F72_SEED20260804") + d50_rows
    component_counts: dict[str, dict[str, int] | None] = {
        "C44_RERUN": None,
        "D50_CONFIRMATION": _component_count(d50),
        "F72_SEED20260804": _component_count(first["retained_metrics"]),
        "F72_SEED20260811": None,
    }
    if second is not None:
        rows.extend(false_positive_rows(second, component="F72_SEED20260811"))
        component_counts["F72_SEED20260811"] = _component_count(
    ...
    if acceptance is not None:
        if c44 is None or acceptance["component_hashes"] != {
                ...
                "d50_confirmation_aggregate_sha256": sha256_json(d50),
                "f72_seed20260804_aggregate_sha256": sha256_json(first),
        }:
            raise C0B5ScoringError("public summary acceptance components differ")
        included_documents = sum(
            component_counts[key]["negative_false_positive_documents"]
            for key in ("C44_RERUN", "D50_CONFIRMATION", "F72_SEED20260804"))
        ...
        if (acceptance["totals"]["negative_false_positive_documents"] !=
                ...
                included_findings):
            raise C0B5ScoringError("public summary final counts differ")
    first_ids = {row["document_id"] for row in rows
                 if row["component"] == "F72_SEED20260804"}
    second_ids = {row["document_id"] for row in rows
                  if row["component"] == "F72_SEED20260811"}
    value = {
        "version": "c0b5-public-summary-v1", **_identity(result),

"""C0B-6 persistence callbacks around the frozen scored executor.

Only the policy-neutral scored request types/function are imported from C0B-4.  C0B-6
...
class C0B6ExecutorError(RuntimeError):
    """Injected storage or resolver evidence violated the frozen C0B-6 contract."""
...
def runtime_event(point: Any, *, event: str, lane_id: str,
    ...
    """Store one self-digested C0B-6 runtime event."""
    if event not in {
            "DISPATCHING", "RAW_VALID", "NORMALIZED_DUPLICATE", "INVALID",
            "ORPHANED", "CANCELLED"}:
        raise C0B6ExecutorError("unknown C0B-6 runtime event")
    header = point.header()
    value = {
        "version": "c0b6-runtime-event-v1",
...
def persist_scored_finish(point: Any, lane_id: str,
    ...
    """Finish one charged attempt, then publish its matching C0B-6 event."""
...
def reconcile_runtime_events(point: Any, resolver: Any) -> None:
    ...
    preflight_ids = {stable_hash({
        "c0b6_preflight": kind, "invocation": ordinal})
    ...
    for attempt in attempts:
        ...
        if owner_id not in resolver.work_ids:
            raise C0B6ExecutorError("attempt owner is outside the frozen plan")

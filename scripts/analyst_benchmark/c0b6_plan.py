"""Pure planning for the C0B-6 assistive review-budget confirmation.
...
from .c0b6_policy import POLICY_ID, POLICY_SHA256
from .c0b6_schema import (
    AcceptancePlan, C0B6PublicWork, CancellationControl, ContextControl,
    HealthControl, LanePlan, MasterPlan,
)

LANE_ORDER = ("F72_20260811", "F72_20260818", "C44_1")
LANE_CONFIG = {
    "F72_20260811": (
        20260811, "F_SEED_20260811", "c0b6-f72-20260811-nonce-v1", 92),
    "F72_20260818": (
        20260818, "F_SEED_20260818", "c0b6-f72-20260818-nonce-v1", 92),
    "C44_1": (1, "F_ACCEPTANCE", "c0b6-acceptance-c44-nonce-v1", 44),
}
SELECTION = {
    "model": "qwen3.6:27b",
...
class C0B6PlanError(RuntimeError):
    """A parent, fixture, candidate, or derived request is not exact."""


@dataclass(frozen=True)
class ResolvedC0B6Control:
    """Exact offline inputs for one frozen seed-20260811 control."""
...
def _sha256(value: Any, label: str) -> str:
    if (type(value) is not str or len(value) != 64
        ...
        raise C0B6PlanError(f"{label} must be lowercase SHA-256")
    return value
...
def _group_id(lane_id: str) -> str | None:
    ...
    return stable_hash({
        "candidate_id": candidate_id(), "domain": "c0b6-stage-f-group-v1",
        "plan_key": plan_key,
    })
...
def _cell_id(lane_id: str, seed: int) -> str:
    ...
    return stable_hash({
        ...
        "chunk_chars": 8000, "domain": "c0b6-public-cell-v1",
...
def _work_id(*, cell_id: str, lane_id: str, row: Mapping[str, Any]) -> str:
    return stable_hash({
        ...
        "domain": "c0b6-public-work-v1", "nonce": row["nonce"],
...
def _validate_work(row: Mapping[str, Any], *, lane_id: str, seed: int,
    ...
    if type(row) is not dict or set(row) != _WORK_KEYS:
        raise C0B6PlanError("generated work has an inexact shape")
    if any((row["stage"] != "F", row["budget_stage"] != "F",
        ...
        raise C0B6PlanError("generated work differs from its frozen configuration")
    ...
    if (type(row["nonce"]) is not str or not row["nonce"].startswith("FENCE_")
            or len(row["nonce"]) != 38):
        raise C0B6PlanError("generated work nonce is invalid")
    expected_group = _group_id(lane_id)
    if row["activation_group_id"] != expected_group:
        raise C0B6PlanError("generated work activation group differs")
    if row["cell_id"] != _cell_id(lane_id, seed):
        raise C0B6PlanError("generated work cell identity differs")
    if row["work_id"] != _work_id(
            cell_id=row["cell_id"], lane_id=lane_id, row=row):
        raise C0B6PlanError("generated work identity differs")
    try:
        return C0B6PublicWork.model_validate(
            row, strict=True).model_dump(mode="json")
    except (TypeError, ValueError) as exc:
        raise C0B6PlanError("generated work violates C0B-6 schema") from exc
...
def _work_rows(lane_id: str, corpus: PublicCorpus,
    ...
    if type(run_nonce_key) is not bytes or len(run_nonce_key) != 32:
        raise C0B6PlanError("run nonce key must contain exactly 32 bytes")
    if lane_id not in LANE_CONFIG:
        raise C0B6PlanError("unknown C0B-6 lane")
    ...
    for doc_id in ids:
        ...
        if document is None:
            raise C0B6PlanError(f"public corpus lacks {doc_id}")
        ...
        if nonce in source:
            raise C0B6PlanError(f"derived nonce occurs in source {doc_id}")
        chunks = chunker.chunk(source, chunk_chars=8000, overlap_chars=256)
        if lane_id == "C44_1" and len(chunks) != 1:
            raise C0B6PlanError(f"C44 document is not exactly one chunk: {doc_id}")
        for item in chunks:
    ...
    if len(rows) != expected or len({row["work_id"] for row in rows}) != expected:
        raise C0B6PlanError(f"{lane_id} work census differs from {expected}")
    return rows


def _control_nonce(key: bytes, source_identity: str, *, domain: str) -> str:
    return _derive_nonce(
        key, domain=domain, view_identity=source_identity, seed=20260811)
...
def _control_plan(first_lane_work: Sequence[Mapping[str, Any]],
    ...
    config_sha256 = sha256_json(_generation_config(20260811))
    ...
    context = {
        "version": "c0b6-context-control-v1", **identity,
        "control_id": stable_hash({
            "domain": "c0b6-context-control-v1", "candidate_id": candidate,
            "config_sha256": config_sha256,
            "payload_sha256": request_spec_hash(context_spec),
        }),
        "kind": "context_probe", "lane_id": "F72_20260811",
        "purpose": "c0b6_stage_f_candidate_context", "candidate_id": candidate,
        ...
        "trigger_rule": "first_bounded_http_terminal_seed20260811",
        "payload_sha256": request_spec_hash(context_spec),
    }
    matches = [row for row in first_lane_work
               if row["doc_id"] == "pos_pii_013" and row["chunk_index"] == 0]
    if len(matches) != 1:
        raise C0B6PlanError("cancellation source work is not unique")
    ...
    if len(chunks) != 1:
        raise C0B6PlanError("cancellation source is not exactly one chunk")
    source = chunks[0].text
    nonces = {
        name: _control_nonce(
            run_nonce_key, source_identity, domain=f"c0b6-{name}-nonce-v1")
        for name in ("cancellation", "health")
    }
    if len({source_work["nonce"], *nonces.values()}) != 3 \
            or any(nonce in source for nonce in nonces.values()):
        raise C0B6PlanError("control nonce/source isolation failed")
    prompts = {name: build_prompt("v2", source, nonce)
               for name, nonce in nonces.items()}
    requests = {name: stable_hash(_request_payload(prompt, 20260811))
                for name, prompt in prompts.items()}
    cancellation = {
        "version": "c0b6-cancellation-control-v1", **identity,
        "control_id": stable_hash({
            "domain": "c0b6-cancellation-control-v1", "candidate_id": candidate,
            "request_sha256": requests["cancellation"],
        }),
        "kind": "cancellation_probe", "lane_id": "F72_20260811",
        "candidate_id": candidate, "seed": 20260811,
    ...
    health = {
        "version": "c0b6-health-control-v1", **identity,
        "control_id": stable_hash({
            "domain": "c0b6-health-control-v1", "candidate_id": candidate,
            "nonce": nonces["health"], "request_sha256": requests["health"],
        }),
        "kind": "cancellation_health", "lane_id": "F72_20260811",
        "candidate_id": candidate, "seed": 20260811,
        "prompt_sha256": hashlib.sha256(prompts["health"].encode()).hexdigest(),
        "source_doc_id": "pos_pii_013", "chunk_index": 0,
        "nonce": nonces["health"],
        "health_work_id": stable_hash({
            "domain": "c0b6-health-work-v1", "candidate_id": candidate,
    ...
    try:
        ...
        raise C0B6PlanError("generated controls violate C0B-6 schema") from exc
...
def build_lane_plan(lane_id: str, *, corpus: PublicCorpus, run_nonce_key: bytes,
    ...
    if corpus.master_manifest_sha256 != manifest or lane_id not in LANE_CONFIG:
        raise C0B6PlanError("lane parent, corpus, or identity is invalid")
    seed = LANE_CONFIG[lane_id][0]
    value = {
        "version": ("c0b6-acceptance-plan-v1" if lane_id == "C44_1"
    ...
    try:
        ...
        raise C0B6PlanError("generated lane plan violates C0B-6 schema") from exc
...
def _validate_lane_plan(value: Mapping[str, Any], corpus: PublicCorpus) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _PLAN_KEYS:
        raise C0B6PlanError("lane plan has an inexact shape")
    lane_id = value.get("lane_id")
    if lane_id not in LANE_CONFIG:
        raise C0B6PlanError("lane plan has an unknown identity")
    ...
    if (value["version"] != version or value["seed"] != seed
        ...
        raise C0B6PlanError("lane plan differs from its frozen identity")
    ...
    if list(dict.fromkeys(row["doc_id"] for row in rows)) != list(order):
        raise C0B6PlanError("lane work order differs from public corpus")
...
def build_master_plan(*, corpus: PublicCorpus, run_nonce_key: bytes,
    ...
    value = {
        "version": "c0b6-master-plan-v1", **_identity(protocol_sha256),
    ...
    try:
        ...
        raise C0B6PlanError("generated master violates C0B-6 schema") from exc


def validate_master_plan(value: Mapping[str, Any], *, corpus: PublicCorpus,
                         run_nonce_key: bytes) -> dict[str, Any]:
    """Independently rebuild and byte-compare a stored C0B-6 master plan."""
    if type(value) is not dict:
        raise C0B6PlanError("master plan must be an exact object")
    ...
    if canonical_json(value) != canonical_json(expected):
        raise C0B6PlanError("master plan differs from independent re-derivation")
    return expected
...
def lane_from_master(master: Mapping[str, Any], lane_id: str, *,
    ...
    if len(matches) != 1:
        raise C0B6PlanError("unknown or duplicate lane")
    return matches[0]
...
def _resolve_owned_work(owner: Mapping[str, Any], row: Mapping[str, Any], *,
    ...
    if document is None or document.document_sha256 != row["document_sha256"]:
        raise C0B6PlanError("work differs from its public document")
    ...
    if view_id != row["view_id"] or row["chunk_index"] >= len(chunks):
        raise C0B6PlanError("work source view or chunk is absent")
    ...
    if (hashlib.sha256(item.text.encode()).hexdigest() != row["chunk_sha256"]
        ...
        raise C0B6PlanError("resolved request differs from frozen work")
...
def resolve_work(master: Mapping[str, Any], work_id: str, *, corpus: PublicCorpus,
    ...
    if len(matches) != 1:
        raise C0B6PlanError("unknown or duplicate work identity")
...
def _resolve_control(parsed: Mapping[str, Any], control: Mapping[str, Any], *,
                     corpus: PublicCorpus, run_nonce_key: bytes) -> ResolvedC0B6Control:
    first = parsed["lane_plans"][0]["payload"]
    sources = [row for row in first["work"]
               if row["doc_id"] == "pos_pii_013" and row["chunk_index"] == 0]
    if len(sources) != 1:
        raise C0B6PlanError("control source work is not unique")
    if control["kind"] == "context_probe":
        ...
        if request_spec_hash(spec) != control["payload_sha256"]:
            raise C0B6PlanError("context control differs from request spec")
        return ResolvedC0B6Control(
    ...
    payload = _request_payload(prompt, 20260811)
    spec = RequestSpec(
    ...
    if (control["nonce"] != nonce
        ...
        raise C0B6PlanError("resolved control differs from frozen request")
    return ResolvedC0B6Control(
        deepcopy(control), spec, sources[0]["work_id"], chunk)


def resolve_controls(master: Mapping[str, Any], *, corpus: PublicCorpus,
                     run_nonce_key: bytes) -> dict[str, ResolvedC0B6Control]:
    parsed = validate_master_plan(master, corpus=corpus, run_nonce_key=run_nonce_key)
...
class C0B6RequestResolver:
    ...
    def __init__(self, master: Mapping[str, Any], *, corpus: PublicCorpus,
        ...
        if len(self.work) != 228:
            raise C0B6PlanError("master work index is duplicate or incomplete")

    def resolve_work(self, work_id: str) -> dict[str, Any]:
        match = self.work.get(work_id)
        if match is None:
            raise C0B6PlanError("unknown C0B-6 work identity")
    ...
    def resolve_controls(self) -> dict[str, ResolvedC0B6Control]:
        return {name: _resolve_control(
...
def build_request_resolver(master: Mapping[str, Any], *, corpus: PublicCorpus,
                           run_nonce_key: bytes) -> C0B6RequestResolver:
    return C0B6RequestResolver(master, corpus=corpus, run_nonce_key=run_nonce_key)

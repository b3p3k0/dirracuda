...
def test_searxng_results_requires_auth(client):
    r = client.get("/api/searxng/results")
    assert r.status_code == 303

def test_searxng_results_ok(logged_in_client, monkeypatch):
    fake_rows = [
        {"result_id": 1, "url": "http://example.com/files/", "probe_status": "clean",
         "verdict": "OPEN_INDEX", "probe_preview": "index of /files"},
    ]
    monkeypatch.setattr(
        "experimental.webui.app.load_searxng_rows",
        lambda db_path: fake_rows,
    )
    r = logged_in_client.get("/api/searxng/results")
    assert r.status_code == 200
    data = r.json()
    assert data["count"] == 1
    assert data["results"][0]["url"] == "http://example.com/files/"


# ---------------------------------------------------------------------------
# /api/searxng/actions/probe
# ---------------------------------------------------------------------------

def test_searxng_probe_requires_auth(client):
    r = client.post("/api/searxng/actions/probe", json={"result_ids": [1]})
    assert r.status_code == 303

def test_searxng_probe_missing_csrf(logged_in_client):
    r = logged_in_client.post("/api/searxng/actions/probe", json={"result_ids": [1]})
    assert r.status_code == 403

def test_searxng_probe_queues_job(logged_in_client, monkeypatch):
    monkeypatch.setattr(
        "experimental.webui.app.probe_searxng_rows",
        lambda db_path, result_ids, **kw: {"processed": len(result_ids), "updated": len(result_ids)},
    )
    csrf = _csrf_from_dashboard(logged_in_client)
    r = logged_in_client.post(
        "/api/searxng/actions/probe",
...
# ---------------------------------------------------------------------------
# /api/searxng/actions/promote
# ---------------------------------------------------------------------------

def test_searxng_promote_requires_auth(client):
    r = client.post("/api/searxng/actions/promote", json={"result_ids": [1]})
    assert r.status_code == 303

def test_searxng_promote_missing_csrf(logged_in_client):
    r = logged_in_client.post("/api/searxng/actions/promote", json={"result_ids": [1]})
    assert r.status_code == 403

def test_searxng_promote_ok(logged_in_client, monkeypatch):
    fake_row = {"result_id": 1, "url": "http://example.com/files/"}
    ...
    monkeypatch.setattr(
        "experimental.webui.app.promote_searxng_rows",
        lambda db_path, rows, result_ids: fake_summary,
    )
    csrf = _csrf_from_dashboard(logged_in_client)
    r = logged_in_client.post(
        "/api/searxng/actions/promote",

...
def auto_detect_database_path(backend_path: Path | str) -> Path:
    """Detect existing DB in backend dir, preferring canonical filename."""
    backend = normalize_backend_path(backend_path)
    ...
    return (backend / CANONICAL_DB_FILENAME).resolve(strict=False)
...
def resolve_database_path(
    ...
    4) default backend/dirracuda.db
    """
    backend = normalize_backend_path(backend_path)

...
# ---------------------------------------------------------------------------
# Tiered fallback chain (Slice 6A extraction gate)
# ---------------------------------------------------------------------------


def test_fallback_smb_ftp_when_http_tables_absent(monkeypatch):
    """Tier-2 fallback: http_servers absent → SMB+FTP rows returned, no crash.

    Explicit assertion that the fallback chain is preserved after the protocol-
    union cluster was extracted to db_protocol_union_engine.py.
    """
    # _make_db never creates http_servers, so this is the tier-2 path every time.
    path = _make_db(smb=True, ftp=True)
    try:
        conn = sqlite3.connect(path)
        conn.execute(
            "INSERT INTO smb_servers (ip_address, country_code, status) VALUES (?,?,?)",
            ("192.168.0.1", "US", "active"),
        )
        conn.execute(
            "INSERT INTO ftp_servers (ip_address, country_code, status) VALUES (?,?,?)",
            ("10.0.0.5", "DE", "active"),
        ...
        # Only S and F rows — no H rows and no crash despite missing http_servers
        assert host_types == {"S", "F"}
        assert any(r["ip_address"] == "192.168.0.1" for r in rows)
        assert any(r["ip_address"] == "10.0.0.5" for r in rows)
    finally:
        os.unlink(path)


def test_fallback_smb_only_when_ftp_tables_absent(monkeypatch):
    """Tier-3 fallback: ftp_servers absent → SMB-only rows returned, no crash.

    Explicit assertion that the tier-3 fallback is preserved after the protocol-
    union cluster was extracted to db_protocol_union_engine.py.
    """
    path = _make_db(smb=True, ftp=False)
    try:
        ...
        conn.execute(
            "INSERT INTO smb_servers (ip_address, country_code, status) VALUES (?,?,?)",
            ("172.16.0.1", "GB", "active"),
        )
        conn.execute(
            "INSERT INTO smb_servers (ip_address, country_code, status) VALUES (?,?,?)",
            ("172.16.0.2", "GB", "active"),
        )
        conn.commit()
        conn.close()
        ...
        assert total == 2
        # All rows are SMB — no FTP or HTTP tables exist, no crash
        assert all(r["host_type"] == "S" for r in rows)
        assert {r["ip_address"] for r in rows} == {"172.16.0.1", "172.16.0.2"}
    finally:
        os.unlink(path)

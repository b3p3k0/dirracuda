...
class TestFtpScanDialogOptionBuild:
    ...
    def test_scan_options_do_not_include_custom_filters(self, tk_root):
        """GUI scan options no longer include per-scan custom filter strings."""
        dlg = _make_dialog(tk_root)
        opts = dlg._build_scan_options()
        assert "custom_filters" not in opts
...
class TestFtpScanManagerOverrides:
    ...
    def test_custom_filters_not_forwarded_to_run_ftp_scan(self):
        """GUI scan manager path does not forward per-scan custom filters."""
        sm = self._make_scan_manager()
...
class TestDashboardFtpWiring:
    ...
    def test_http_button_opens_dialog(self):
        """HTTP button click in idle state → show_http_scan_dialog called."""
        import gui.components.dashboard as dash_module
        from gui.components.dashboard import DashboardWidget

        mock_self = self._make_dashboard_mock("idle")

        with patch.object(dash_module, "show_http_scan_dialog") as mock_dialog:
            DashboardWidget._handle_http_scan_button_click(mock_self)

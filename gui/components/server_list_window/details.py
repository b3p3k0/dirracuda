...
def show_server_detail_popup(parent_window, server_data, theme, settings_manager=None,
    ...
    cached_probe = _load_probe_result_for_detail(
        server_data,
        settings_manager=settings_manager,
    )
    ...
    def _open_browse_window() -> None:
        ...
        # Resolve a DatabaseReader for Shodan banner lookup (mirrors HTTP branch pattern).
        # _open_browse_window is the fallback path; primary route is browse_callback.
        _db = None
        if settings_manager:
            try:
        ...
        open_smb_browser(
            ...
            theme=theme,
            settings_manager=settings_manager,
            on_extracted=(None if not settings_manager
                          else getattr(parent_window, "_handle_extracted_update", None)),
        )
...
def _load_probe_result_for_detail(
    ...
    *,
    ...
    """Load the full probe snapshot for the active detail row."""
    ip_address = server_data.get("ip_address", "")
    host_type = str(server_data.get("host_type", "S") or "S").upper()
    port = server_data.get("port") if host_type in {"F", "H"} else None
    protocol_server_id = server_data.get("protocol_server_id")
    ...
    db_path = None
    if settings_manager is not None and hasattr(settings_manager, "get_database_path"):
        try:
            db_path = settings_manager.get_database_path()
        except Exception:
            db_path = None
    ...
    return load_probe_result_for_host(
        ip_address,
        host_type,
        port=port,
    )
...
def _start_probe(
    ...
    host_type = str(server_data.get("host_type") or "S").strip().upper()
    ip_address = server_data.get('ip_address')
    if not ip_address:

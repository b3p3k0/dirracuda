...
(country/region, API key, concurrency, timeouts, TLS toggle, verbose).
Uses the same region map and country validation logic as ScanDialog.

Differences from FtpScanDialog:
- No auth_timeout (HTTP has no auth step)
- No access_concurrency (single discovery worker pool)
- listing_timeout → request_timeout (HTTP page request)
- Adds allow_insecure_tls toggle (default True — locked decision)
- No TemplateStore wiring in Card 1 (deferred to avoid namespace collision risk)
...
class HttpScanDialog:
    ...
    timeouts and API key) are passed through to the HTTP scan worker
    via the existing temporary-config-override mechanism in scan_manager (Card 2+).
    """
    ...
    def __init__(
        ...
        self.allow_insecure_tls_var = tk.BooleanVar(value=True)  # locked decision default
    ...
    def _create_dialog(self) -> None:
        ...
        self.dialog.title("Start HTTP Scan")
        self.dialog.geometry("1010x790")
    ...
    def _create_header(self) -> None:
        ...
        self.theme.create_styled_label(frame, "🌐 Start HTTP Scan", "heading").pack(anchor="w")
        self.theme.create_styled_label(
            frame,
            "Configure and start a new HTTP scan to discover open directory-index servers.",
            "body",
    ...
    def _create_max_results_option(self, parent: tk.Frame) -> None:
        ...
        heading = self._create_accent_heading(container, "Max Shodan Results")
        heading.pack(fill=tk.X)
        ...
        entry = tk.Entry(
            row,
            textvariable=self.http_max_results_var,
            width=10,
            font=self.theme.fonts["body"],
        )
    ...
    # ------------------------------------------------------------------
    # TLS toggle (HTTP-specific, locked decision)
    # ------------------------------------------------------------------

    def _create_tls_option(self, parent: tk.Frame) -> None:
        """Create TLS verification toggle. allow_insecure_tls defaults to True."""
        container = tk.Frame(parent)
        self.theme.apply_to_widget(container, "card")
        container.pack(fill=tk.X, padx=15, pady=(0, 10))

        self._create_accent_heading(container, "🔒 HTTPS TLS Verification").pack(fill=tk.X)

        cb = tk.Checkbutton(
            container,
            text="Allow insecure HTTPS (self-signed / untrusted certs)",
            variable=self.allow_insecure_tls_var,
            font=self.theme.fonts["small"],
        )
        self.theme.apply_to_widget(cb, "checkbox")
        cb.pack(anchor="w", padx=12, pady=(6, 2))

        info_label = self.theme.create_styled_label(
            container,
            "When checked, HTTPS targets with invalid certificates are still verified.\n"
            "Uncheck to skip targets that fail strict TLS validation.",
            "small",
            fg=self.theme.colors["text_secondary"],
        )
    ...
    def _create_bulk_probe_option(self, parent: tk.Frame) -> None:
        """Create optional post-scan probe checkbox (Card 5 compatibility placeholder)."""
        container = tk.Frame(parent)
        self.theme.apply_to_widget(container, "card")
        container.pack(fill=tk.X, padx=15, pady=(0, 10))
        ...
        info_label = self.theme.create_styled_label(
            container,
            "Automatically run probe on discovered HTTP hosts after scan.",
            "small",
            fg=self.theme.colors["text_secondary"],
        )
        info_label.pack(anchor="w", padx=15, pady=(0, 5))
    ...
    def _create_concurrency_options(self, parent: tk.Frame) -> None:
        ...
        rows = [
            ("Discovery workers:", self.discovery_concurrency_var, "Hosts verified in parallel"),
        ]
    ...
    def _create_timeout_options(self, parent: tk.Frame) -> None:
        ...
        self._create_accent_heading(container, "⏱ HTTP Timeouts (seconds)").pack(fill=tk.X)

        validate_cmd = self.dialog.register(self._validate_integer_input)

        timeout_rows = [
            ("Connect timeout:", self.connect_timeout_var, "TCP connect"),
            ("Request timeout:", self.request_timeout_var, "HTTP response read"),
        ]
    ...
    def _create_button_panel(self) -> None:
        ...
        start_btn = tk.Button(btns, text="🚀 Start HTTP Scan", command=self._start)
    ...
    # ------------------------------------------------------------------
    # Country validation (mirrors ScanDialog / FtpScanDialog exactly)
    # ------------------------------------------------------------------
    ...
    def _build_scan_options(self) -> Dict[str, Any]:
        """
        Return a scan_options dict for scan_manager.start_http_scan() (Card 2+).

        country is None for global scans; comma-separated codes otherwise.
        Key names follow HTTP module conventions.
        """
        ...
        request_timeout = self._parse_positive_int(
        ...
        if self._settings_manager is not None:
            try:
                ...
                self._settings_manager.set_setting("http_scan_dialog.region_africa", self.africa_var.get())
                self._settings_manager.set_setting("http_scan_dialog.region_asia", self.asia_var.get())
                self._settings_manager.set_setting("http_scan_dialog.region_europe", self.europe_var.get())
                self._settings_manager.set_setting(
                    "http_scan_dialog.region_north_america", self.north_america_var.get()
                )
                self._settings_manager.set_setting("http_scan_dialog.region_oceania", self.oceania_var.get())
                self._settings_manager.set_setting(
                    "http_scan_dialog.region_south_america", self.south_america_var.get()
                )
            except Exception:
                # Best-effort only; do not block scan start on settings write failures.
                pass

        http_cap = max(1, _cap_coerce_int(self.http_max_results_var.get(), 100, minimum=1, maximum=100000))
        ...
        try:
            persist_query_budget_state(self._settings_manager, {"http_max_shodan_results_per_scan": http_cap})
        except Exception:
            pass
        ...
        return {
            ...
            # Fixed flags — both protocols are always attempted (Card 4 consumer reads these).
            "verify_http": True,
            "verify_https": True,
            # TLS behavior — only user-visible toggle (locked decision).
            "allow_insecure_tls": allow_insecure_tls,
            "verbose": verbose,
            "bulk_probe_enabled": bool(self.bulk_probe_enabled_var.get()),
            "smb_max_shodan_results_per_scan": cross_proto["smb_max_shodan_results_per_scan"],
            "ftp_max_shodan_results_per_scan": cross_proto["ftp_max_shodan_results_per_scan"],
            "http_max_shodan_results_per_scan": http_cap,
            "smb_max_query_credits_per_scan": cross_proto["smb_max_query_credits_per_scan"],
    ...
    def _start(self) -> None:
        ...
        scan_desc = f"protocol: HTTP; target: {country_desc}"
